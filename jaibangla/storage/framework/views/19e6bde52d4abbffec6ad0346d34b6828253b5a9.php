...
<?php $__env->startSection('content'); ?>
  <div class="content-wrapper">
    ...
    <section class="content-header">
      <h1>
        <?php if($is_verifier == 1): ?>
          De-Activate Beneficiary
        <?php elseif($is_approver == 1): ?>
        ...
        <div class="modal-dialog">
          <div class="modal-content">
            ...
            <div class="modal-body">
              <!-- <form method="POST" action="<?php echo e(url('resume-ben-payment')); ?>" id="resume_form"> -->
                <!-- <?php echo e(csrf_field()); ?> -->
...
<script>
  $(document).ready(function(){
    ...
    $('#submit_btn').click(function(){
      ...
      if( error_select_type != '' || error_scheme_type != '' /*|| error_is_rural_urban !='' || error_block_ulb !='' */|| error_ben_id != '' || error_ben_fname != ''){
        ...
        var table=$('#example').DataTable( {
          ...
          "ajax": 
          {
            url: "<?php echo e(url('search-using-id-name')); ?>",
            type: "post",
            data:function(d){
    ...
    $('.client-js-urban').change(function() {
      ...
      $getDistCode=\App\Configduty::where('user_id',Auth::user()->id)->value('district_code');
       ...
       ?>
      select_district_code= "<?php echo e($getDistCode); ?>";
      ...
      $.confirm({
        ...
        buttons: {
          confirm: function () {
            ...
            $.ajax({
              type: 'post',
              url: "<?php echo e(route('lppPausePaymentDetails')); ?>",
              data: { id:benId, scheme_id:schemeIdLpp, _token: '<?php echo e(csrf_token()); ?>' },
      ...
      $.confirm({
        ...
        buttons: {
          confirm: function () {
            ...
            $.ajax({
              type: 'post',
              url: "<?php echo e(route('lppResumePaymentDetails')); ?>",
              data: { id:benId, scheme_id:schemeIdLpp, _token: '<?php echo e(csrf_token()); ?>' },
              success: function (response) {
                $('#loadingDiv').hide();
                // console.log(response);
    ...
    //               type: 'post',
    //               url: "<?php echo e(route('lppResumePaymentDetails')); ?>",
    //               data: { ben_id:Lppid, scheme_id:resumeScheme, lot_generate_no:lotGene, resume_month:resumeMonth, _token: '<?php echo e(csrf_token()); ?>' },
    //               success: function (response) {
    //                 $('.loadingDivModal').hide();
    //                 // console.log(response);
    //                 if (response.status == 1) {
  ...
  function editFunction(value, scheme_id){
    ...
    if ($('#select_item_update_'+value).val() == '') {
      ...
      if (op == 'bank') {
        ...
        $.ajax({
          type: 'post',
          url: "<?php echo e(route('getModalDataUpdateStop')); ?>",
          data: { op_type:op, scheme_id:scheme_id, id:value, _token: '<?php echo e(csrf_token()); ?>' },
          success: function (response) {
            $('#loadingDiv').hide();
            // console.log(response);
            if (response.status == 1) {
        ...
        $.ajax({
          type: 'post',
          url: "<?php echo e(route('getModalDataUpdateStop')); ?>",
          data: { op_type:op, id:value, scheme_id:scheme_id, _token: '<?php echo e(csrf_token()); ?>' },
          success: function (response) {
            $('#loadingDiv').hide();
            // console.log(response);
            if (response.status == 1) {
  ...
  $(document).on('click', '#verifySubmit', function(){     
    ...
    else
    {
      ...
      if ((new_acc == old_acc) && (new_ifsc == old_ifsc)) {
        ...
        $.ajax({
          type: 'POST',
          url: "<?php echo e(route('updateBenBankDetails')); ?>",
          data: fd,
          processData: false,
          contentType: false,
          success: function (response) {
  ...
  $(document).on('click', '#stopSubmit', function(){
    ...
    else
    {
      ...
      $.ajax({
        type: 'POST',
        url: "<?php echo e(route('stopPaymentBenDetails')); ?>",
        data: formData,
        dataType: 'json',
        processData: false,
        contentType: false,
  ...
  function mobileUpdate(formData){
    ...
    $.ajax({
      type: 'POST',
      url: "<?php echo e(route('updateMobileBenDetails')); ?>",
      data: formData,
      dataType: 'json',
      processData: false,
      contentType: false,
  ...
  function View_encolser_modal(doc_name,doc_type,is_profile_pic){
    ...
    $.ajax({
      url: "<?php echo e(route('ajaxViewPassbook')); ?>",
      type: "POST",
       data: {
       doc_type: doc_type,
