...
@section('action-content')
    ...
    <div class="row" style="margin-bottom:1%">
              ...
              @foreach ($districtList as $districtarr)
              <option value="{{$districtarr->district_code}}">{{trim($districtarr->district_name)}}</option>
              @endforeach
              }
              @endif
          ...
          <div class="row">
              ...
              class="btn btn-success">Filter</button></div>
                ...
                <button type="button" name="reset" id="reset" class="btn btn-warning">Reset</button></div>
        ...
        <div class="col-md-12" id="reportbody" style="margin-top: 2%;">
          ...
          <form action="BulkAccept" method="post" id="register_form">
            <input type="hidden" name="_token" id="token" value="{{ csrf_token() }}">
            <button type="button"  style="margin: 0% 0% 2% 0%;" type="button" name="bulk_approve" id="confirm_bulk" value="approve" class="btn btn-info col-sm-3 col-xs-5 btn-margin" disabled>
              Accept as valid Aadhar
              </button>
             <input type="hidden" name="scheme_id" id="scheme_id" value="{{$pr1}}">
            <div id="register_form_checkboxes">
        ...
        {{-- <form method="post" id="register_form" action="{{url('BulkAccept')}}" class="submit-once">
        ...
        <thead>
        <tr role="row"> 
        <th width="9%" class="text-left">Beneficiary ID</th>
        <th width="20%" class="text-left">Beneficiary Name</th>
        <th width="5%" class="text-left">Aadhaar NO</th>  
    ...
    <div id="confirmaccept" class="modal fade">
  
  
      <div class="modal-dialog modal-confirmaccept">
        <div class="modal-content">
          ...
          <h4 class="modal-title w-100">Do you really want to Accept?</h4>	
           
             
          </div>
          <div class="modal-footer justify-content-center">
            <button type="button" class="btn btn-secondary" data-dismiss="modal">Cancel</button>
            <button type="submit" class="btn btn-info" id="confirm_accept" >OK</button>
  ...
  <!-- Start Accept Model -->
  <form method="post" id="approval_form" action="{{url('benAccept-noaadhar')}}" class="submit-once"> --}}
    <input type="hidden" name="_token" value="{{ csrf_token() }}">
  <div class="modal fade" id="ben_accept_modal" tabindex="-1">
    <div class="modal-dialog ">
      <div class="modal-content">
        <div class="modal-header btn-success">
          ...
          <h4 class="modal-title ">Accept as valid Aadhar </h4>
        </div>
        <div class="modal-body">
          <h4>Are you sure you want to Accept the valid Aadhar no. of the beneficiary details mentioned below?</h4><hr/>

          <table style="width:100%">
            <tr>
              <td style="width:30%;"><span class="item_header">Beneficiary Id:</span></td>
              <td><span class="item_value" name="accept_ben_id"  id="accept_ben_id"></span></td>
            </tr>
            <tr>
              <td><span class="item_header">Beneficiary Name:</span></td>
              <td><span class="item_value"  name="accept_ben_name" id="accept_ben_name"></span></td>
            </tr>
            <tr>
              <td><span class="item_header">Aadhar No:</span></td>
              <td><span class="item_value" id="accept_aadhar_no"></span></td>
            </tr>
          ...
          <input type="hidden" name="accept_beneficiary_id" id="accept_beneficiary_id"/>
        </div>
       
        <div class="modal-footer">
          <button type="button" class="btn btn-secondary" data-dismiss="modal">Cancel</button>
          <button type="submit" class="btn btn-info" id="confirm_yes" >Accept</button>
...
$('#reject_Button').click(function(e){
  ...
  $.ajax({
    ...
    success: function (datas) {
      alert('Beneficiary with Application Id '+$('#reject_ben_id').text()+' Successfully rejected');
...
var table=$('#example').DataTable( {
      ...
      "paging": true,
      "pageLength":20,
      "lengthMenu": [[20, 50, 80, 120, 150, 180], [20, 50, 80, 120, 150, 180]],
  "serverSide": true,
  "deferRender": true,
      "processing":true,
      "bRetrieve": true,
    ...
    exportOptions: {
      ...
      columns: [0,1,2,3,4]
        ...
        title: 'Accept allow No Aadhar List for scheme: "{{$scheme_name}}"',
        messageTop: function () {
    ...
    table.on('click','.ben_accept_button',function(){
      $tr = $(this).closest('tr');
      // if(($tr).hasClass('child')){
      //   $tr = $tr.prev('parent');
      ...
      // }
      var data = table.row($tr).data();
      $('#accept_beneficiary_id').val(data['id']);
      $('#accept_ben_id').html(data['id']);
      $('#accept_ben_name').html(data['ben_name']);
      $('#accept_ben_father_name').html(data['benf_name']);
      $('#accept_ben_ifsc').html(data['bank_ifsc']);
      $('#accept_ben_accno').html(data['bank_code']);
      $('#accept_aadhar_no').html(data['aadhar_no_rw']);
      $('#ben_accept_modal').modal('show');
    });
});

function controlCheckBox(){

  const checkedValues = $('input[type=checkbox][name="approvalcheck[]"]:checked').map(function() {
    return $(this).val();
  }).get();

// Now, checkedValues contains an array of values of all checked checkboxes with the name 'approvalcheck[]'


  $('#register_form_checkboxes').html(`<input type="hidden" name="check_beificieries"  value='${checkedValues.join()}'/>`);


    if (checkedValues.length) {
      $("#confirm_bulk").show();
      document.getElementById('confirm_bulk').disabled = false;
    } else{
      $("#confirm_bulk").hide();
      document.getElementById('confirm_bulk').disabled = true;
    }
  }
