            ...
            <div class="tab-content" style="margin-top:16px;">
             ...
             <div class="tab-pane active" id="personal_details">
              <div class="panel panel-default">
               <div class="panel-heading"><h4><b>Duare Sarkar VII Camps Marking Report</b></h4></div>
...
<div id="divScrool"> 
         ...
         <thead>
              ...
              <th>Total Quota(C)</th>
              <th>Verification Pending(D)</th>
              <th>Mark as Duare Sarkar VII Camps(E)</th>
              <th>Mark as Duare Sarkar VII but Approval Pending(F)</th> 
              <th>Mark as Duare Sarkar VII and also Approved(G)</th> 
...
$(document).ready(function(){
  ...
  $(".exportToExcel").click(function(e){
    ...
    exclude: ".noExl",
    name: "Worksheet Name",
    filename: "Jai Bangla Duare Sarkar VII Camps Marking Mis Report "+c_time, //do not include extension
  ...
  // alert(select_year);
     ...
     $('#search_details').hide();
        $.ajax({
                type: 'get',
                dataType:'json',
                url: '{{ url('oapsmdsmarkoMisReportPost') }}',
                data: {
                  scheme_id: scheme_id,
                  district: district,
                ...
                success: function (data) {
                  ...
                  if(data.return_status){
                   ...
                   $.each(data.row_data, function(i, item) {
                     var capacity = isNaN(parseInt(item.capacity)) ? 0 : parseInt(item.capacity);
                     var verification_pending = isNaN(parseInt(item.verification_pending)) ? 0 : parseInt(item.verification_pending);  
                     var total_mark = isNaN(parseInt(item.total_mark)) ? 0 : parseInt(item.total_mark);          
                     var mark_with_pending = isNaN(parseInt(item.mark_with_pending)) ? 0 : parseInt(item.mark_with_pending);          
                     var mark_and_pending = isNaN(parseInt(item.mark_and_pending)) ? 0 : parseInt(item.mark_and_pending);          

                     fotter_1=fotter_1+capacity;
                     fotter_2=fotter_2+verification_pending;
                     fotter_3=fotter_3+total_mark;
                     fotter_4=fotter_4+mark_with_pending;
                     fotter_5=fotter_5+mark_and_pending;
                   
                     table.append("<tr><td>"+(i+1)+"</td><td>"+item.location_name+"</td><td>"+capacity+"</td><td>"+verification_pending+"</td><td>"+total_mark+"</td><td>"+mark_with_pending+"</td><td>"+mark_and_pending+"</td></tr>");
                      //slno++;
