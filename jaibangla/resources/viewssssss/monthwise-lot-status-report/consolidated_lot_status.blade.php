              ...
              <table id="example" class="display" cellspacing="0" width="100%" style="border-top: 1px solid #e6e6e6;">
                <thead>
                  <tr role="row" style="font-size: 14px;">
                    <th  width="10%" class="sorting_asc" tabindex="0" aria-controls="example2" rowspan="1" colspan="1" aria-label="Name: activate to sort column descending" aria-sort="ascending">Scheme Name</th>
                    ...
                    <th width="10%" class="sorting_asc" tabindex="0" aria-controls="example2" rowspan="1" colspan="1" aria-label="Name: activate to sort column descending" aria-sort="ascending">Total Beneficiary</th>
                    <th width="10%" class="sorting_asc" tabindex="0" aria-controls="example2" rowspan="1" colspan="1" aria-label="Name: activate to sort column descending" aria-sort="ascending">Response Pending</th>
                    <th width="10%" class="sorting_asc" tabindex="0" aria-controls="example2" rowspan="1" colspan="1" aria-label="Name: activate to sort column descending" aria-sort="ascending">Get Lot No. Response Pending</th>
                    <th width="10%" class="sorting" tabindex="0" aria-controls="example2" rowspan="1" colspan="1" aria-label="Email: activate to sort column ascending">No. of Success</th>
                    <th width="10%" class="sorting" tabindex="0" aria-controls="example2" rowspan="1" colspan="1" aria-label="Email: activate to sort column ascending">No. of Failed</th>
                    <th width="10%" class="sorting" tabindex="0" aria-controls="example2" rowspan="1" colspan="1" aria-label="Email: activate to sort column ascending">Eligible For Next Month</th>
                    <th width="10%" class="sorting" tabindex="0" aria-controls="example2" rowspan="1" colspan="1" aria-label="Email: activate to sort column ascending">Not Eligible For Next Month</th>
                    <th width="10%" class="sorting" tabindex="0" aria-controls="example2" rowspan="1" colspan="1" aria-label="Email: activate to sort column ascending">Repeat Lot Done</th>
                    <th width="10%" class="sorting" tabindex="0" aria-controls="example2" rowspan="1" colspan="1" aria-label="Email: activate to sort column ascending">Pending For Repeat Lot</th>
                    <th width="10%" class="sorting" tabindex="0" aria-controls="example2" rowspan="1" colspan="1" aria-label="Email: activate to sort column ascending">Get Pending For Repeat Lot No.</th>                
                  </tr>
                </thead>
...
<script>
    ...
    var table = $('#example').DataTable( {
      ...
      "ajax": 
      {
        ...
        "columns": [
            ...
            { "data": "get_lot_response_pending" },
            { "data": "success" },
            { "data": "failed" },
            { "data": "eligible_for_next_month" },
            { "data": "not_eligible_for_next_month" },
            { "data": "repeat_lot_done" },
            { "data": "pending_for_repeat_lot" },
            { "data": "get_lot_pending_for_repeat_lot" },
          ],             
     
      buttons: [
       {
           extend: 'pdf',
           title: 'Monthly Lot Status Report <?php echo date('d-m-Y'); ?>',
  ...
  function getResponsePendingLot(value){
    var url = 'get_response_pending_lot_details';
    genericLotDetails(url,value);
  }
  function getRepeatPendingLot(value){
    var url = 'get_repeat_pending_lot_details';
    genericLotDetails(url,value);
  }  

  function genericLotDetails(url,value){
    ...
    $.ajax({
      ...
      success: function(datas)
      {
        ...
        if (datas.pay_mode == 'IFMS') {
          html += '<thead><tr><th>SL. No</th><th>Lot No</th><th>Lot Created</th><th>Total Beneficiary</th></tr></thead><tbody>';
        }
        else if (datas.pay_mode == 'SBI') {
          html += '<thead><tr><th>SL. No</th><th>Debit Reference</th><th>Lot Created</th><th>Total Beneficiary</th></tr></thead><tbody>';
        }
        for (var i = 0; i < datas.data.length; i++) {
          var count =count+1;
          if (datas.pay_mode == 'IFMS') {
            var total_ben = total_ben + datas.data[i].ben_count;
            html += '<tr><td>'+count+'</td><td>'+datas.data[i].lot_no+'</td><td>'+dateFormart(datas.data[i].created_at)+'</td><td>'+datas.data[i].ben_count+'</td></tr>';
          }
          else if (datas.pay_mode == 'SBI') {
            var total_ben = total_ben + datas.data[i].ben_count;
            html += '<tr><td>'+count+'</td><td>'+datas.data[i].debit_reference+'</td><td>'+dateFormart(datas.data[i].created_at)+'</td><td>'+datas.data[i].ben_count+'</td></tr>';
          }
          else {
        ...
        html += '<tr style="background-color : #cfffcc; "><td></td><td></td><td><b>Total</b></td><td><b>'+total_ben+'</b></td></tr>'; 
        html += '</tbody></table>';
        $('#lot_details').html(html);
        $('#modalTitle').text('Response Pending Lot Details');
