      ...
      <div class="modal-dialog modal-lg" id="printArea">
        <div class="modal-content">
          <div class="modal-header">
            ...
            <h3 class="modal-title">Beneficiay Detail View</h3>
          </div>
          <div class="modal-body">
            ...
            <div class="row">
                <div class="col-md-4">
                  <span class="item_header">Activity Name:</span> <span class="item_value" id="ben_activity"></span>
                </div>
                <div class="col-md-4">
                  <span class="item_header">Folk Form:</span> <span class="item_value" id="ben_folk_form"></span>
                </div>
                <div class="col-md-4">
                  <span class="item_header">Experience:</span> <span class="item_value" id="ben_exp"></span>
                </div> 
            </div>
    ...
    <div class="modal fade" id="ben_edit_modal" tabindex="-1">
			<div class="modal-dialog ">
              ...
              <tr>
					...
					<div class="modal-footer">
          <form class="row" method="POST" action="{{ route('lpp-singlestep.application-edit-view') }}">
                <button type="button" class="btn btn-primary" data-dismiss="modal">Close</button>
          ...
          <div class="modal-body">
            <div class="row">
              ...
              <div class="col-md-offset-2 col-md-8 panel panel-info">
                  ...
                  <div class="row">
                    <div class="col-md-12"><span class="item_header">Fathers Name: </span>
                    <span class="item_value" id="selective_edit_benf_name"></span></div>
                  </div>
                  <div class="row">
                    <div class="col-md-5"><span class="item_header required-field">Date of Birth: </span>
                    <input type="date" name="selective_edit_ben_dob" id="selective_edit_ben_dob" class="form-control"  tabindex="5" value="{{old('selective_edit_ben_dob')}}" />
                  ...
                  <div class="row">
                    <div class="col-md-12"><span class="item_header">Address Line 1: </span>
                      <span class="item_value" id="selective_edit_ben_addr"></span></div>
            ...
            <div class="row">
              <div class="col-md-3">
                <span class="item_header">Mobile No:</span> 
                <input type="text" class="item_value bg-blue" id="selective_edit_ben_mobile" name="selective_edit_ben_mobile"/>
              </div>
  ...
  $('#edit_approve_button').click(function(){
    ...
    $ben_mobile = $.trim($('#selective_edit_ben_mobile').val());
    $ben_dob = $.trim($('#selective_edit_ben_dob').val());
    ...
    if($ben_fname=='' || $ben_bank_ifsc=='' || $ben_bank=='' || $ben_bank_branch=='' || $ben_bank_account =='' || ben_dob ==''){
      alert('Mandatory Field is empty');
      return false;
    }
    else{
      var today = new Date();      
      var diff_ms = new Date(today).getTime() - new Date($ben_dob).getTime();
      var age_dt = new Date(diff_ms); 
      var age = Math.ceil(age_dt.getUTCFullYear() - 1970);

      $.ajax({
        type: 'POST',
        url: '{{ url('lpp-singlestep/editBeneficiary') }}',
        data: {
          _token: '{{ csrf_token() }}',
          scheme: "{{ $scheme }}",
        ...
        success: function (datas) {
          ...
          alert('Beneficiary Detail Successfully Updated and Approved');
        },
        error: function (ex) {
  ...
  $('.urban').change(function() {
      ...
      $.ajax({
        type: 'POST',
        url: '{{ url('lpp-singlestep/loadLocalBody') }}',
        data: {
          _token: '{{ csrf_token() }}',
  ...
  $('#bulk_approve').click(function(){
    ...
    $.ajax({
      type: 'POST',
      url: '{{ url('lppSingleStepBenBulkApprove') }}',
      dataType: 'json',
      data: {
        _token: '{{ csrf_token() }}',
        scheme: "{{ $scheme }}",
  ...
  //   $.ajax({
  //     type: 'POST',
  //     url: '{{ route('pensionform.application-edit-view') }}',
  //     data: {
  //       id: $('#reject_beneficiary_id').val(),
  //       scheme: "{{ $scheme }}",
  //       _token: '{{ csrf_token() }}',
  //     },
  //     success: function (datas) {
  ...
  $('#reject_Button').click(function(e){
    ...
    $.ajax({
      type: 'POST',
      url: '{{ url('lppSingleStepBenReject') }}',
      data: {
  ...
  var table=$('#example').DataTable( {
        ...
        "ajax": 
        {
			url: "{{ url('getLppSingleStepVerifyData') }}",
			type: "POST",
          	data:function(d){
	   ...
	   table.on('click','.ben_view_button',function(){
          ...
          $('#ben_activity').html(data['activity_name']);
          $('#ben_folk_form').html(data['folk_form']);
          $('#ben_exp').html(data['experience_in_year']+' years');
