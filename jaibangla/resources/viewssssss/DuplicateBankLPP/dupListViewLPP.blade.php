<style type="text/css">
  ...
  <div class="content-wrapper">
    ...
    <section class="content">
      <div class="box box-default">
        ...
        <div class="row">
        <div class="col-md-2">
          <a href="{{ url('deDupBankListLPP') }}"><img width="50px;" style="pull-left" src="{{ asset("images/back.png") }}" alt="Back" /></a></div>
                <div class="col-md-8">
              ...
              <div class="table-responsive">
                 <table id="example" class="display" cellspacing="0" width="100%"> 
                  ...
                  @foreach ($data as $row )
                    ...
                    <td><button class="btn btn-info ben_doc_button btn-sm" id="btnDoc_{{$row['application_id']}}" value="{{$row['application_id']}}_{{$row['scheme_id']}}">View Bank Passbok</button>&nbsp;
                    @if($row['allowed']==1)
                    <button class="btn btn-warning ben_reject_button btn-sm" id="btnUpdate_{{$row['application_id']}}" value="{{$row['application_id']}}">Reject</button>&nbsp;
                    <a class="btn btn-danger btn-sm" href="{{ url('/dedupBankLPPUpdate')}}?bank_code={{$bank_code}}&application_id={{$row['application_id']}}&scheme_id={{$row['scheme_id']}}">Edit</a>&nbsp;
                     @endif
                    </td>
                   </tr>
                  @endforeach
  ...
  <div id="modalApproval" class="modal fade">
    <form method="POST" action="{{ route('dupBankRejectLPP')}}"  name="dupReject" id="dupReject">
  ...
  $(document).ready(function() {
    ...
    var splitValues = benidArr.split('_');
    var benidArr = splitValues[0];
    var scheme_id = splitValues[1];
   

    View_encolser_modal('Copy of Bank Pass book', 10, benidArr,scheme_id);
});
  ...
  function View_encolser_modal(doc_name,doc_type,application_id,scheme_id){
   // alert(is_faulty);
  $('#encolser_name').html('');
