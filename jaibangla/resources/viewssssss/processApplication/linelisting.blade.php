...
<body class="hold-transition skin-blue sidebar-mini">
  <div class="wrapper">
    ...
    <div class="content-wrapper">
      ...
      <form method="post" id="register_form" action="{{url('BulkApprovenoaadhar')}}" 
                    class="submit-once">
      ...
      <input type="hidden" name="action_type" id="action_type" value="1"/>
       ...
       <table id="example" class="display" cellspacing="0" width="100%"> 
        ...
        <thead>
            ...
            <tr>
              <td>{{$data_item->id}}</td>
              <td>{{$data_item->ben_fname}}</td>
              <td>{{$data_item->mobile_no}}</td>
              <td>{{$data_item->dob}}</td>
              @if($verifier_type=='Subdiv' || $verifier_type=='District')
              <td>{{$data_item->block_ulb_name}}</td>
              @endif
              <td>{{$data_item->gp_ward_name}}</td>
              <td><a href="benDetails/{{$data_item->id}}/{{$data_item->scheme_id}}" class="btn btn-xs btn-primary"><i class="glyphicon glyphicon-edit"></i>View</td>
              @if($verifier_type=='District')
 ...
 <div class="row">
            ...
            <div class="col-sm-7">
               ...
               <?php echo $data->appends(array('scheme_id' => $scheme_id))->links(); ?>
              </div>
            </div>
  </div>
  </div>
