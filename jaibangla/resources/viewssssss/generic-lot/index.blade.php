                        ...
                        <div class="form-group{{ $errors->has('select_lot_type') ? ' has-error' : '' }}">
                            ...
                            <div class="col-md-6">
                                <select name="select_lot_type" id="select_lot_type"
                                    ...
                                    @if($lot_type->id!='7' && $lot_type->id!='8' && $lot_type->id!='10')
                                   @php
                                   $arr_scheme=[];
                                       foreach($reports as $report){
                                           array_push($arr_scheme,$report->id);
                                       }
                                       //print_r( $arr_scheme);die;
                                   @endphp 
                                   <!-- Stopped fresh lot for duare sarkar phase 4 21052022 -->
                                   @if(in_array(3,$arr_scheme) )
                                   @if ( $lot_type->id!=9 )
                                   <option value="{{ $lot_type->id}}" @if (Session::get('lotTypeSession')==$lot_type->id) selected @endif >{{$lot_type->lot_type}}</option>
                                   @endif
                                   @else
                                    <!-- Stopped fresh lot for duare sarkar phase 4 21052022 -->
                                   @if(!empty(array_intersect([2,8,9,10,11], $arr_scheme))  && $lot_type->id==3)
                                   
                                    <option value="{{ $lot_type->id}}" >{{$lot_type->lot_type}}</option>
                                   
                                    @endif
                                    @if ( $lot_type->id!=9 &&  $lot_type->id!=10)  
                                   <option value="{{ $lot_type->id}}" @if (Session::get('lotTypeSession')==$lot_type->id) selected @endif >{{$lot_type->lot_type}}</option>
                                   @endif
                        ...
                        <div class="form-group{{ $errors->has('select_year') ? ' has-error' : '' }}">
                            ...
                            <div class="col-md-6">
                                <select name="select_year" id="select_year" required class="form-control select2"
                                    ...
                                    <option value="2022-2023" @if (Session::get('yearSession')=='2022-2023') selected @endif>2022-2023</option>
                                    <option value="2023-2024" @if (Session::get('yearSession')=='2023-2024' ) selected @endif>2023-2024</option>
                                </select>
                            </div>
                        </div>
