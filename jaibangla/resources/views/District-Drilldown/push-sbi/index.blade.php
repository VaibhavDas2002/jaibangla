      ...
      <table id="example" class="display" cellspacing="0" width="100%">
        <thead>
          <tr role="row" style="font-size: 12px;">
            ...
            <th width="40%" class="sorting" tabindex="0" aria-controls="example2" rowspan="1" colspan="1" aria-label="Email: activate to sort column ascending">Action</th>                      
          </tr>
        </thead>
        <tbody>
         <?php
          ...
          @foreach($datas as $result)
            ...
            <tr role="row" class="odd">
              <td class="sorting_1" style="text-align: center;">{{$result->lot_no}}</td>
              <td>{{$result->scheme_name}} </td>   
              <td class="sorting_1" style="text-align: center;">{{$result->lot_month}}</td> 
               <td class="sorting_1" style="text-align: center;">{{$result->lot_year}}</td>
              <td class="sorting_1" style="text-align: center;">
                <form class="row" method="POST" action="{{ route('push-to-sbi.showlist') }}">
                  <input type="hidden" name="lot_no" value="{{$result->lot_no}}">
                  <input type="hidden" name="scheme_id" value="{{$result->scheme_id}}">
                  <input type="hidden" name="_token" value="{{ csrf_token() }}">    
                  <button type="submit" class="" >{{$result->credit_count}}</button>
                </form>
              </td>
              <td>
                @if($result->lot_status==1)
                  <form method="POST" action="{{ route('push-to-sbi.export') }}" class="submit-once" 
                    ...
                    @if($result->scheme_id == 3)
                      <input type="hidden"  name="scheme_id" value="{{$result->scheme_id}}"> <!--name="party_code" value="026"--><!-- changed on 04.04.2020 -->
                    @elseif($result->scheme_id == 1)
                      <input type="hidden"  name="scheme_id" value="{{$result->scheme_id}}"> <!--name="party_code" value="028"--><!-- changed on 04.04.2020 -->
                    @endif
                    <div class="col-md-9">
                      <select class="form-control select2" name="bank_account" required>
                        <option value="-1">Please Select Account No</option>
                        @foreach ($bank_accounts as $bank_account)
                          <option value="{{$bank_account->bank_account_no}}:{{$bank_account->ifsc_code}}">{{$bank_account->bank_account_no}} ({{$bank_account->ifsc_code}})</option>
                        @endforeach
                      </select>    
                    </div>
                    <div class="col-md-3">        
                      <button  style="border:1px solid black ;margin: 0% 0% 2% 0%; position: absolute;" type="submit" name="bulk_approve" id="bulk_approve" value="approve" class="btn btn-info  btn-margin" >Push To SBI
                      </button>
                    </div>
                  </form>
                @endif
