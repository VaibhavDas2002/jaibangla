...
.view-details-btn:hover {
    background-color: #cffccd; /* Change to your desired hover background color */
}
  </style>
...
<div class="wrapper">
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      <h1>
       Account/Payment Validation MIS Report
      </h1>
      <ol class="breadcrumb">
              ...
              @endif -->
               ...
               <div class="form-group col-md-4">
                ...
                <div class="form-group col-md-4">
                  ...
                  <select name="failed_type" id="failed_type" class="form-control" tabindex="1" >
                   <option value="">--Select Failed Type--</option>
  ...
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      ...
      <div class="modal-body" id="modalBeneficiaryList">
...
$(document).ready(function(){
  ...
  $(".exportToExcel").click(function(e){
    ...
    exclude: ".noExl",
    name: "Worksheet Name",
    filename: "Account Validation Report for the scheme "+scheme_name+" as on "+c_date, //do not include extension
...
$(document).on('click', '.view-details-btn', function() {
         var beneficiaryList = $(this).data('beneficiary');
         var scheme_id = $('#scheme_id').val();
         var district = $('#district').val();
         var data= {'_token': '{{csrf_token()}}',   'beneficiaryList': beneficiaryList,'scheme_id':scheme_id,'district':district };
         redirectPostExcel('{{route("accountValidationDownloadExcel")}}', data, 'post');
...
function showModal(beneficiaryList) {
    $('#myModal').modal('show');
    $('#modalBeneficiaryList').html(beneficiaryList);
}
function loadDataTable() {
    ...
    $.ajax({
        type: 'post',
        dataType: 'json',
        url: '{{ url('accountValidationGetData') }}',
        data: {
            scheme_id: scheme_id,
            district: district,
        ...
        success: function (data) {
          console.log(data);
            if (data.return_status) {
                ...
                var fotter_2 = 0;
                var fotter_3 = 0;
                tableHead.append("<tr><th>Sl No.</th><th id='location_id'></th><th>Correction Pending Below 1 months</th><th>Correction Pending Between 1-3 months</th><th>Correction Pending Above 3 months</th></tr>");
                $.each(data.row_data, function (i, item) {
                    var total1 = isNaN(parseInt(item.pending_below_1)) ? 0 : parseInt(item.pending_below_1);
                    var total2 = isNaN(parseInt(item.pending_between_1_3)) ? 0 : parseInt(item.pending_between_1_3);
                    var total3 = isNaN(parseInt(item.pending_above_3)) ? 0 : parseInt(item.pending_above_3);
                    fotter_1 = fotter_1+total1;
                    fotter_2 = fotter_2+total2;
                    fotter_3 = fotter_3+total3;

                    var iconHtml1 = '<i class="	fa fa-file-excel-o view-details-btn" data-beneficiary="' + 1 + '_' + JSON.stringify(item.location_id) + '_'+ $('#failed_type').val() +'"title="Download Beneficiary List"></i>';
                    var iconHtml2 = '<i class="fa fa-file-excel-o view-details-btn" data-beneficiary="' + 2 + '_' + JSON.stringify(item.location_id) + '_'+ $('#failed_type').val() +'"title="Download Beneficiary List"></i>';
                    var iconHtml3 = '<i class="fa fa-file-excel-o view-details-btn" data-beneficiary="' + 3 + '_' + JSON.stringify(item.location_id) + '_'+ $('#failed_type').val() +'"title="Download Beneficiary List"></i>';
                    table.append("<tr><td>" + (i + 1) + "</td><td>" + item.location_name + "</td><td>" + item.pending_below_1 + "&nbsp;&nbsp; &nbsp;" + iconHtml1 + "</td><td>" + item.pending_between_1_3 + "&nbsp;&nbsp; &nbsp;" + iconHtml2 + "</td><td>" + item.pending_above_3 + "&nbsp;&nbsp; &nbsp;" + iconHtml3 + "</td></tr>");
                });
                $("#example > tfoot #fotter_id").html("<th></th><th>Total</th><th>" + fotter_1 + "</th><th>" + fotter_2 + "</th><th>" + fotter_3 + "</th>");
