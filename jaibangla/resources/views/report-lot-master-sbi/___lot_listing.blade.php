          ...
          @php $i=1; @endphp
            ...
            <tr>
              ...
              @if($report->lot_status==0 or $report->lot_status==10)
                  <form  method="POST" action="{{ route('push-to-sbi-single-lot') }}" class="submit-once" onSubmit="if(!confirm('Please click on OK if you are sure to export the Lot to SBI')){return false;}">
                      <td>  
                        <input type="hidden" name="_token" value="{{ csrf_token() }}">
	                <input type="hidden" name="lot_no" value="{{$report->lot_no}}">
                  ...
                  @elseif($report->lot_status==4 or $report->lot_status==50)
                    <form id="sbi_payment_submit" method="GET" action="{{ route('sbi_payment_status') }}" class="submit-once" >
				    <td>
                      <input type="hidden" name="lot_no" value="{{$report->lot_no}}">
                      <input type="hidden" name="scheme_id" value="{{$report->scheme_id}}">
                      <input type="hidden" name="debit_ref" value="{{$report->debit_reference}}">
                      ...
                      <input type="hidden" name="_token" value="{{ csrf_token() }}">
                      <button id="btn_submit" type="button" class="btn btn-warning btn-margin">
...
$.confirm({
    ...
    type: 'red',
    icon: 'fa fa-warning',
    content: 'Please click on Confirm if you are sure to import the SBI Payment Response',
    buttons: {
        confirm: function () {
          $('#loadingDiv').show();
          document.getElementById("sbi_payment_submit").submit();
        },
        cancel: function () {
