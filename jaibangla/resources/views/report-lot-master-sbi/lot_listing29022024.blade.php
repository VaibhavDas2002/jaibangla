  ...
  <table id="example" class="display" cellspacing="0" width="100%">
    ...
    <thead>
      <tr role="row" class="sorting_asc" style="font-size: 12px;">
        ...
        <th width="10%" class="sorting" tabindex="0" aria-controls="example2" rowspan="1" colspan="1"
          aria-label="Email: activate to sort column ascending">Debit Remarks</th>
    ...
    <tbody>
      ...
      <tr>
        ...
        @elseif($report->lot_status ==7) <td style="text-align: center">
      ...
      if (select_scheme == '' || lot_year == '' || lot_month == '') {
        ...
        $.ajax({
          ...
          success: function(result) {
            ...
            // New 27-11-2021
            var contents= '';
            contents = '<div><font style="font-size: 15px; font-weight: bold;" class="text-success">Scheme : '+result.datas.data[0].scheme_name+'<br> Lot No : '+result.datas.data[0].lot_no+'<br> Lot Year : '+result.datas.data[0].lot_year+'<br> Lot Month : '+result.datas.data[0].lot_month+'</font></div>';
            contents += '<div><input type="hidden" name="lot_no" id="push_lot_no" value="'+result.datas.data[0].lot_no+'"><input type="hidden" name="scheme_id" id="push_scheme_id" value="'+result.datas.data[0].scheme_id+'"><label for="bank_account">Select Bank Account</label><select class="form-control" name="bank_account" id="bank_account" required><option value="'+acc+':'+ifsc+'">'+acc+' ('+ifsc+')'+'</option></select><span id="error_bank_acc" class="text-danger"></span></div>';
            $.confirm({
