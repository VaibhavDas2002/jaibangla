  ...
  @section('action-content')
                ...
                <tr role="row">
                 @if($app_type=='P')
                  <td width="3%" class="text-right">
                    <input type="checkbox" class="select_all" name="select_all" onchange="document.getElementById('bulk_approve').disabled = !this.checked;">
                  </td>
                  @endif
    ...
    <!-- CLOSED TEMPORARY -->
		<div class="modal fade" id="ben_reject_modal" tabindex="-1">
			<div class="modal-dialog ">
				<div class="modal-content">
					<div class="modal-header">
						...
						<h4 class="modal-title"><span class="reject_revert_txt">Reject</span> Beneficiary Application</h4>
					</div>
					<div class="modal-body">
						<h4>Are you sure you want to <span class="reject_revert_txt">reject</span> the application with the beneficiary details mentioned below?</h4><hr/>
            ...
            <input type="hidden" id="revert_reject"/>

					</div>
					<div class="modal-footer">
						<button type="button" class="btn btn-primary" data-dismiss="modal">Close</button>
						<button type="button" class="btn btn-danger" id="reject_Button" data-dismiss="modal"><span class="reject_revert_txt">Reject</span></button>
					</div>
				</div>
			</div>
		</div>
 ...
 $('#urban_code').change(function() {
  ...
  $('#reject_Button').click(function(e){
    ...
    $.ajax({
      ...
      data: {
        ...
        revert_reject: $('#revert_reject').val(),
        app_type: "{{ $app_type }}",
        _token: '{{ csrf_token() }}',
      },
      success: function (data) {
          if(data.return_status){
          if($('#revert_reject').val()==1)
          var revert_reject_txt="Rejected";
          else
           var revert_reject_txt="Reverted";
           alert('Beneficiary with id '+$('#reject_beneficiary_id').val()+' '+revert_reject_txt);
            $('#example').DataTable().ajax.reload();
          }
        else{
          //$('#loaderdiv').hide();
  ...
  var table=$('#example').DataTable( {
        ...
        "ajax": 
        {
			url: "{{ url('getSingleStepVerifyDataFarmer') }}",
			type: "POST",
          	data:function(d){
				d._token= "{{csrf_token()}}",
       	d.app_type= "{{ $app_type }}"
     ...
     //console.log(table);
	   ...
	   table.on('click','.ben_view_button',function(){
          ...
          $('#ben_id').val(data['id']);
          $('#ben_name').html(data['ben_fullname']);
      ...
      table.on('click','.ben_reject_button',function(){
        ...
        $('.reject_revert_txt').text('');
        $('.reject_revert_txt').text('Reject');
      
        $('#reject_beneficiary_id').val(data['ben_id']);
        $('#revert_reject').val(1);
        $('#reject_ben_id').html(data['application_id']);
        $('#reject_ben_name').html(data['ben_fullname']);
