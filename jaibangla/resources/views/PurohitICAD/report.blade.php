  ...
  @section('action-content')
      ...
      <section class="content">
        ...
        <div class="box-body">
              ...
              @if($approved_rejected == 0)
              Approved Applications
              @elseif($approved_rejected == -1)
              Rejected Applications
              @elseif($approved_rejected == 106)
              Recommended Applications
              @elseif($approved_rejected == 107)
              Verified Applications
              @elseif($approved_rejected == null)
              Fresh Applications
              @endif
              </span></h4>
        ...
        <table id="example" class="display" cellspacing="0" width="100%">
          ...
          <thead>
                <tr>                 
                  ...
                  <!-- <th width="12%" class="text-left">Caste</th> -->
                  <th class="text-left">Application Phase</th>     
                  <th class="text-left">Purohit Type</th>
                  <th class="text-left">Village/Town/City</th>
                  <th>Action</th> 
              </tr>
          </thead>
          <tfoot>
      ...
      <div class="modal-dialog modal-lg" id="printArea">
        <div class="modal-content">
          ...
          <div class="modal-body">
            ...
            <div class="row">
              <div class="col-md-6">
                <span class="item_header">Application Phase:</span> <span class="item_value" id="ben_app_phase"></span>
              </div>
              <div class="col-md-6">
                <span class="item_header">Temple Type:</span> <span class="item_value" id="ben_temple_type"></span>
              </div>
  ...
  var table=$('#example').DataTable( {
        ...
        "ajax": 
        {
			url: "{{ url('getPurohitsVerifyProcessedData') }}",
			type: "POST",
          	data:function(d){
				...
				d._token= "{{csrf_token()}}",
				d.scheme = "{{ $scheme }}",
        d.approved_rejected = "{{$approved_rejected}}"
			}
    } ,
        "columns": [
                  ...
                  { "data": "app_phase","defaultContent":"0" },
                  { "data": "temple_type","defaultContent":"" },
      ...
      text: '<b><i class="fa fa-file-pdf-o" style="color: #3e943d;"></i> PDF</b>',
          ...
          title: 'Purohit Beneficiaries List',
          messageTop: function () {
                var message = "";
                if("{{ $district_name}}" != ""){
      ...
      text: '<b><i class="fa fa-file-excel-o" style="color: #161c9c;"></i> Excel</b>',
        ...
        format: {
            body: function ( data, row, column, node ) {
                ...
                return column === 3 || column === 0  ? "'"+data: data;
            }
        }
			},
          title: 'Purohit Beneficiary Report',
          messageTop: function () {
            var message = "";
            if("{{ $district_name}}" != ""){
              var message = message +"District: {{ $district_name}}, ";
      ...
      text: '<b><i class="fa fa-print" style="color: #d44317;"></i> Print</b>',
	   ...
	   table.on('click','.ben_view_button',function(){
          ...
          $('#benf_name').html(data['ben_fname']);
          $('#ben_dob').html(data['dob']);
          $('#ben_gender').html(data['gender']);
          
          $('#ben_app_phase').html(data['app_phase']);
          $('#ben_temple_type').html(data['temple_type']);
      ...
      //   $('#reject_ben_father_name').html(data['ben_fname']);
