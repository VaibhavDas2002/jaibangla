...
@section('action-content')
    ...
    <section class="content">
          ...
          <table id="example2" class="table table-bordered table-hover dataTable" role="grid" aria-describedby="example2_info">
            ...
            @foreach ($reports as $report)
                <tr role="row" class="odd">
                  ...
                  <td class="sorting_1">{{ $report->first_name }} {{ $report->middle_name }} {{ $report->last_name }} </td>
                  <td class="sorting_1">{{ $report->guardian_name }}</td>
                  <td>{{ $report->appointing_authority }}</td>
                   <td>{{ $report->posting_level }}</td>
                  <td>{{ $report->posting_place }}</td>
                  <td></td>
      ...
      <div class="row">
        <div class="col-sm-5">
          <div class="dataTables_info" id="example2_info" role="status" aria-live="polite">Showing 1 to {{count($reports)}} of {{count($reports)}} entries</div>
        </div>
        <div class="col-sm-7">
