  ...
  <div class="wrapper">
    ...
    <div class="content-wrapper">
      ...
      <section class="content-header">
        <h1>Scheme: {{$scheme}}</h1>
        <font size="3"><span>Month: {{$month}}, Year: {{$year}}</span></font>        
      </section>

      <!-- Main content -->
      <section class="content">
        <div class="box box-default">
          <div class="box-header with-border"><font size="3">Report Lot Master</font></div>
            <div class="box-body">
	         <div>
       ...
       <table id="example" class="display" cellspacing="0" width="100%"> 
        ...
        <thead>
          <tr role="row" class="sorting_asc" style="font-size: 12px;">
            ...
            <th width="5%" class="sorting" tabindex="0" aria-controls="example2" rowspan="1" colspan="1" aria-label="Email: activate to sort column ascending" style="text-align: center">File Name</th>      
            ...
            <tr>
              ...
              <form method="POST" action="{{ route('lot_payment_xls_generate') }}">
                <td style="text-align: center">
                  ...
                  <button class="btn btn-xs btn-margin" onmouseover="$(this).toggleClass('btn-primary');" onmouseout="$(this).toggleClass('btn-primary');" style="font-size: 16px;" title="Total Beneficiary - {{ $report->ben_count }}">
                    {{ $report->ben_count }}
                  </button>
                </td>
              </form>
              ...
              <form method="POST" action="{{ route('lot_payment_xls_generate') }}">
                <td style="text-align: center;">
                  ...
                  <button class="btn btn-xs btn-margin" onmouseover="$(this).toggleClass('btn-danger');" onmouseout="$(this).toggleClass('btn-danger');" style="font-size: 16px;" title="IFMS Wrong Data - @php if($report->ifms_wrongdata_count == '') {print '0';} else {print $report->ifms_wrongdata_count;} @endphp">
                    @php if($report->ifms_wrongdata_count == '') {print '0';} else {print $report->ifms_wrongdata_count;} @endphp
                  </button>
                </td>
              </form>
              @else
              ...
              <form method="POST" action="{{ route('lot_payment_xls_generate') }}">
                <td style="text-align: center">
                  ...
                  <button class="btn btn-xs btn-margin" onmouseover="$(this).toggleClass('btn-danger');" onmouseout="$(this).toggleClass('btn-danger');" style="font-size: 16px;" title="RBI Failed - @php if($report->rbi_failed_count == '') {print '0';} else {print $report->rbi_failed_count;} @endphp">
             ...
             @if($report->rbi_success_count != '')
              <form method="POST" action="{{ route('lot_payment_xls_generate') }}">
                <td style="text-align: center">
                  ...
                  <button class="btn btn-xs btn-margin" onmouseover="$(this).toggleClass('btn-success');" onmouseout="$(this).toggleClass('btn-success');" style="font-size: 16px; " title="RBI Success - @php if($report->rbi_success_count == '') {print '0';} else {print $report->rbi_success_count;} @endphp">
              ...
              @if($report->lot_status==1 and $report->ref_no!=-1 and $report->ack_status!=-1)
                ...
                @elseif($report->lot_status==0 and $report->push_to_ifms_status ==1 and $report->dotdone_status =='' and $report->ref_no!=-1 and $report->ack_status!=-1)
                  ...
                  <form  method="POST" action="{{ route('ack_status') }}" class="submit-once" onSubmit="if(!confirm('Please click on OK if you are sure that the Lot has been billed in IFMS and submitted to treasury')){return false;}">
                  <td>
                      <input type="hidden" name="lot_no" value="{{$report->lot_no}}">
                      <input type="hidden" name="scheme_id" value="{{$report->scheme_id}}">
                        <input type="hidden" name="lot_year" value="{{$report->lot_year}}">
                  ...
                  @elseif($report->lot_status==0 and $report->push_to_ifms_status ==1 and $report->dotdone_status==1 and $report->ack_status==1 and $report->voucher_no =='' and $report->ref_no!=-1 and $report->ack_status!=-1)
					  @if($report->rbi_flag)
					  <form method="POST" action="{{ route('rbi_payment_status') }}" class="submit-once" onSubmit="if(!confirm('Please click on OK if you are sure to import the RBI Report')){return false;}">
				  ...
				  @else
					  ...
					  <form method="POST" action="{{ route('rbi_payment_status') }}" class="submit-once" onSubmit="if(!confirm('Please click on OK if you are sure to import the RBI Report again!')){return false;}">
					  <!--<form method="GET" action="{{ route('wrong_file_test') }}" class="submit-once" onSubmit="if(!confirm('Please click on OK if you are sure to import the RBI Report')){return false;}">-->
					  <td>
						  <input type="hidden" name="lot_no" value="{{$report->lot_no}}">
						  <input type="hidden" name="scheme_id" value="{{$report->scheme_id}}">
  ...
  $(document).ready(function() {
    ...
    $('#example').DataTable( {
      ...
      buttons: [
       {
           extend: 'pdf',
           title: 'Lot Master Report <?php echo date('d-m-Y'); ?>',
           messageTop: 'Scheme : {{$scheme}}\n Year : {{$year}}\n Month : {{$month}}',
