...
<body class="hold-transition skin-blue sidebar-mini">
          ...
          <div class="box-header with-border">
            <div class="row">
              <div class="col-sm-6">
                ...
                @if ($is_urban == 1)
          <span class="text-red">District: {{ $districtObj->district_name }}</span>
        @elseif ($is_urban == 2)
      <span class="text-red">District:{{ $districtObj->district_name}}</span>
    @endif
              </div>
              ...
              <div class="col-md-6 ">
                <span class="text-danger" style="font-size: 13px; float: right; font-weight: bold;">Note: Please click
                  the checkbox for enter the capacity</span>
              </div>
            </div>
          </div>
          <div class="box-body">
            ...
            <form method="POST" action="{{ url('add-capacity') }}" name="capacityForm" id="capacityForm">
              <div class="form-group special-quota">
                ...
                <label class="radio-inline">
                  <input type="radio" name="special_quota" value="0" id="quota_no" checked> No
                </label>
              </div>
              {{ csrf_field() }}
              ...
              @if ($is_urban != 0)
          <input type="hidden" name="dist_code" id="dist_code" value="{{$districtObj->district_code}}">
        @endif

              <div class="table-responsive">
                <table id="example" class="display table table-responsive table-hover" cellspacing="0" width="100%"
              ...
              aria-label="state: activate to sort column ascending">District</th>
            @elseif($is_urban == 1)
          <th width="30%" tabindex="0" aria-controls="example2" rowspan="1" colspan="1"
          aria-label="state: activate to sort column ascending">Sub-Division</th>
        @elseif($is_urban == 2)
        <th width="30%" tabindex="0" aria-controls="example2" rowspan="1" colspan="1"
        aria-label="state: activate to sort column ascending">Block</th>
      @endif
                      ...
                      <th width="30%" tabindex="0" aria-controls="example2" rowspan="1" colspan="1"
                        aria-label="Action: activate to sort column ascending">Capacity [Existing Quota]</th>
                      <th width="2%" tabindex="0" aria-controls="example2" rowspan="1" colspan="1"
                        aria-label="Action: activate to sort column ascending"
                        style="background: linear-gradient(to right, #ffccff 0%, #ccffff 100%);"><span
                          class="fa fa-check-square"></span></th>
                      <th width="38%" tabindex="0" aria-controls="example2" rowspan="1" colspan="1"
                        aria-label="Action: activate to sort column ascending"
                        style="background: linear-gradient(to right, #ccffff 0%, #ffccff 100%);">Enter Capacity [New
                        Quota]</th>
                    </tr>
                  </thead>
        ...
        <tr role="row" class="odd">
          ...
          <td>{{ $c->capacity }}</td>
          <td>
          <input type="checkbox" name="s_cap_{{$c->code}}" id="s_cap_{{$c->code}}"
          value="_{{$c->name}}_{{$c->code}}" class="ben_checkbox">
          <font class="text-primary"><b></b></font>
          </td>
          <td>
          <input type="hidden" name="dist_arr[]" id="dist_arr_{{$c->code}}" disabled>
          <input type="text" name="capacity[]" id="cap_{{$c->code}}" disabled class="textbox_pre_css"
      ...
      @endforeach
        ...
        <tr role="row" class="odd">
          ...
          <td>
          <input type="hidden" name="urban_arr[]" id="urban_arr_{{$c->code}}" disabled>
          <input type="text" name="capacity[]" id="cap_{{$c->code}}" disabled class="textbox_pre_css"
          onkeyup="if (/\D/g.test(this.value)) this.value = this.value.replace(/\D/g,'')"
          autocomplete="off">
          </td>
      ...
      @endforeach
        ...
        <tr role="row" class="odd">
          ...
          <td>
          <input type="hidden" name="block_arr[]" id="block_arr_{{$c->code}}" disabled>
          <input type="text" name="capacity[]" id="cap_{{$c->code}}" disabled class="textbox_pre_css"
          onkeyup="if (/\D/g.test(this.value)) this.value = this.value.replace(/\D/g,'')"
          autocomplete="off">
          </td>
      ...
      @endforeach
          @endif
              ...
              <div align="center">
                <input type="submit" class="btn btn-primary btn-lg" name="ben_pre_save" id="ben_pre_save" value="Add"
    ...
    <div class="modal-dialog">
      <div class="modal-content">
        <div class="modal-header">
          ...
          <h4 class="modal-title">{{$scheme_name}} Capacity</h4> @if ($is_urban == 1)
        <span class="text-green">Level : {{$districtObj->district_name}}(Sub-Division)</span>
      @elseif ($is_urban == 2)
      <span class="text-green">Level :{{$districtObj->district_name}} (Block)</span>
    @else
      <span class="text-green">Level : (District)</span>
    @endif
        </div>
  ...
  <script>
    ...
    $('#ben_pre_save').on('click', function (e) {
      ...
      var is_urban = $('#is_urban').val();
      var values;

      // Get values based on the selected area type
      if (is_urban == 0) {
        values = $("input[name='dist_arr[]']").map(function () { return $(this).val(); }).get();
      } else if (is_urban == 1) {
        values = $("input[name='urban_arr[]']").map(function () { return $(this).val(); }).get();
      } else if (is_urban == 2) {
        values = $("input[name='block_arr[]']").map(function () { return $(this).val(); }).get();
      }
      ...
      // Generate table header based on the type of area
      if (is_urban == 0) {
        ...
        html += '<table id="preview_tbl" class="table table-bordered table-condensed"><tr style="background-color: #e6e6ff;"><th>Sub-Division</th><th>New Quota Added</th></tr>';
      } else if (is_urban == 2) {
        html += '<table id="preview_tbl" class="table table-bordered table-condensed"><tr style="background-color: #e6e6ff;"><th>Block</th><th>New Quota Added</th></tr>';
      }



      // Populate table rows with the values
      ...
      // Append the generated table to the details div and display the modal
      ...
      $('#final_submit').off('click').on('click', function () {
        if ($('#preview_tbl tr').length == 1) {
          // Display an alert if no data rows are present
          alert('Please add at least one entry with capacity.');
        } else {
