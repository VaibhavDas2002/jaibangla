        ...
        <div class="col-md-12">
          ...
          <div> <!-- class="box box-primary" -->
            ...
            <form method="post" id="register_form" action="{{url('lb-wcd-search')}}"  class="submit-once" >
             ...
             <input type="hidden" name="urban_code" id="urban_code" value="{{$urban_code}}">
            <div class="tab-content" style="margin-top:16px;">
             ...
             @if($level=='SUBDIV')
                ...
                <select name="block" id="block" class="form-control" tabindex="11" >
                  ...
                  @foreach($munc_arr as $munc)
                  <option value="{{$munc->code}}" @if( old('block') == $munc->code)  selected  @endif >{{$munc->location_name}}</option>
                  @endforeach   
              ...
              @endif
                <div class="form-group col-md-4" id="divBodyCode">
                            <label class="required-field">{{$gp_ward_txt}}</label>

                            <select name="gp_ward" id="gp_ward" class="form-control"
                              tabindex="15">
                              <option value="">--Select --</option>
                              @foreach($gp_ward_arr as $gp_ward)
                             <option value="{{$gp_ward->code}}" @if( old('block') == $gp_ward->code)  selected  @endif >{{$gp_ward->location_name}}</option>
                              @endforeach   
                  ...
                  <div class="form-group col-md-4" id="divBodyCode">
                            <label class="required-field">Search By</label>

                            <select name="search_by" id="search_by" class="form-control"
                              tabindex="20">
                              <option value="">--Select --</option>
                              <option value="1">Family Id</option>  
                              <option value="2">Aadhar Number</option> 
                              <option value="3">Ration Card Number</option>
                              <option value="4">Epic/Voter Id</option>
                            </select>
                            <span id="error_search_by" class="text-danger"></span>
                          </div>

                        <div class="form-group col-md-4" id="search_textDiv" style="display:none;">
                            <label class="required-field" id="search_txt_label">Search Text</label>

                             <input type="text" name="search_text" id="search_text" class="form-control"
                            placeholder="" maxlength="50" tabindex="25" />
                            <span id="error_search_by" class="text-danger"></span>
                          </div>
                        </div>
...
$('#search_by').change(function() {
  var code=$(this).val();
  if(code==1){
    $("#search_txt_label").text("Family Id");
   $("#search_textDiv").show();
  }
  else if(code==2){
    $("#search_txt_label").text("Aadhar Number");
    $("#search_textDiv").show();
  }
  else if(code==3){
    $("#search_txt_label").text("Ration Card Number");
   $("#search_textDiv").show();
  }
  else if(code==4){
    $("#search_txt_label").text("Epic/Voter ID");
   $("#search_textDiv").show();
  }
  else{
    $("#search_txt_label").text("Search Text");
 $("#search_textDiv").hide();
  }
});
