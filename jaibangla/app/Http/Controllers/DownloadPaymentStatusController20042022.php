...
class DownloadPaymentStatusController extends Controller
{
    ...
    public function paymentStatusGenerateExcel(Request $request){
        ...
        $ben_success = [];
        $ben_error = [];
    	$result = DB::select(DB::raw("(select be.pension_id,be.name,lm.lot_month,lm.lot_year,be.acc_no as account_no,be.ifsc as ifsc,be.mobile_no,
			...
			(select id from pension.beneficiary where created_by_local_body_code=".$rural_urban." and next_level_role_id in (0,-2,-99) and scheme_id=".$scheme.")
			...
			(select id from pension.beneficiary where created_by_local_body_code=".$rural_urban." and next_level_role_id=0 and scheme_id=".$scheme.") and tld.status_code=tc.code 
