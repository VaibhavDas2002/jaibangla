...
class LifeCertificateController extends Controller
{
    ...
    if (request()->ajax()) {
      ...
      return datatables()->of($data)->setTotalRecords($totalRecords)
        ...
        ->addColumn('application_id', function ($data) use ($scheme_id, $scheme_length, $id_length) {
          ...
          return $app_id;
        })->addColumn('view', function ($data) use ($scheme_id, $is_verifier, $is_operator) {
  ...
  public function editUnlock(Request $request)
  {
    ...
    $already_uploaded = 0;
    $encolserdata = DB::table($schema . '.ben_docs')->where('ben_id', $id)->where('is_active', true)->where('doc_type_id', $this->life_certificate_id)->first();
    // dd( $encolserdata);
  ...
  function editLifeCertificatePost(Request $request)
  {
    ...
    $doc_certificate = DocumentType::select('id', 'doc_size_kb', 'doc_name', 'doc_type', 'doucument_group')->where("id", $this->life_certificate_id)->first();
    $already_uploaded = 0;
    $encolserdata = DB::table($schema . '.ben_docs')->where('ben_id', $id)->where('doc_type_id', $this->life_certificate_id)->where('is_active', true)->first();
    if (!empty($encolserdata)) {
      $already_uploaded = 1;
      $required = 'nullable';
    } else {
    ...
    if ($validator->passes()) {
      ...
      $is_saved1 = DB::table($schema . '.beneficiaries')->where('id', $id)->where('id', $id)->where('created_by_dist_code', $distCode)->update($input);
      ...
      if ($is_saved1 && $is_saved2 && $is_saved3 && $is_saved_log) {
        DB::commit();
  ...
  public function SingleApproveLifeCertificate(Request $request)
  {
    ...
    if ($request->action_type == 'Back to Operator') {
      ...
      $accept_reject_model->created_by_dist_code = $district_code;
      $accept_reject_model->ip_address = request()->ip();
      $accept_reject_model->op_type = 'WT';
      $accept_reject_model->application_id = $id;
      $accept_reject_model->op_type = class_basename(Route::current()->controller) .'@'. Route::getCurrentRoute()->getActionMethod() .'WT';

      $input = ['next_level_role_id_edit' => NULL, 'unlock_status' => 1];
      $is_saved_log = $accept_reject_model->save();
      $is_saved1 = DB::table($schema . '.beneficiaries')->where('id', $id)->where('created_by_dist_code', $district_code)->update($input);
