				...
				foreach ($remote_xml_file as $key => $value) {
					...
					foreach ($value as $key2[$j] => $val2[$j]) {
						...
						if ($j == 10) {
							...
							DB::table('ifms.transaction_lot_details')->where('pension_id', $pension_id)->where('drn_part', $DRN_part)->where('scheme_id', $scheme_id)/*->where('acc_no', $acc_no)->where('ifsc', $ifsc)->where('mobile_no', $mob_no)*/->update(['ifms_status' => $val2[$j], 'ifms_ref_no' => 0, 'wrongdata_flag' => 1, 'updated_at' => DB::raw("now(),"), 'status_code' => 'IF' /*, 'paid_yymm' => 0*/]);
						}
						$j = $j + 1;
					}
