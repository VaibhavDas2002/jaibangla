  ...
  public function getDuplicateAadharListView(Request $request)
  {
    if ($request->ajax()) {
      ...
      if ($mapping_level == 'Block') {
        $query = "select * from " . $table_name . " where (next_level_role_id > 0 or next_level_role_id is null or next_level_role_id = 0 or next_level_role_id =-97) and ((dup_aadhar = 1 and dup_aadhar_edit_role_id IS NULL ) or (dup_mobile = 1 and dup_mobile_edit_role_id IS NULL)) and scheme_id = " . $scheme_id . " and created_by_dist_code = " . $district_code . " and created_by_local_body_code = " . $urban_body_code . " ";
      ...
      if ($validator->passes()) {
        ...
        try {
          ...
          if ($is_upload == 1) {
            ...
            if ($function_call[0]->dup_adjustment_update == 1) {
              UpdateBenDetails::insert($updateBenDetailsData);
              if ($remarks != '') {
                DB::table($beneficiary_table)->where('scheme_id', $scheme_id)->where('id', $ben_id)->update(['dup_aadhar_edit_remarks' => $remarks]);
              }
            }
            else {
              DB::rollback();
        ...
        try {
          ...
          if ($is_insert) {
            ...
            if ($function_call[0]->dup_adjustment_update == 1) {
              UpdateBenDetails::insert($updateBenDetailsData);
              if ($remarks != '') {
                DB::table($beneficiary_table)->where('scheme_id', $scheme_id)->where('id', $ben_id)->update(['dup_mobile_edit_remarks' => $remarks]);
              }
            }
            else {
              DB::rollback();
