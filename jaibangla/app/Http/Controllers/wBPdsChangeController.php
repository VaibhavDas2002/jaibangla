...
class wBPdsChangeController extends Controller
{
      ...
      if ($request->hasFile('doc_' . $doc_type_id)) {
        ...
        if (strtolower($mime_type) == 'image/jpeg') {
          if ($u_extension == 'jpg' || $u_extension == 'jpeg') {
            ...
            $errors = array();
            $errorMsg = "You are trying to upload an incorrect file for " . $doc_row->doc_name;
            array_push($errors, $errorMsg);
            return redirect("/Viewpdsnamemismatch?scheme_id=" . $scheme_id . "&type=" . $type . "&id=" . $request->id)->with('errors', $errors);
          }
        ...
        $function_parameters = "in_beneficiary_id => " . $id . ",
          ...
          in_doc_type_name => '" . $doc_row->doc_name . "',
          in_datetime => '" . $c_time . "'";
  ...
  function aadharNameValidMIS(Request $request)
  {
    ...
    if ($designation_id_old == 'Admin' || $designation_id_old == 'HOD' || $designation_id_old == 'HOP' ||  $designation_id_old == 'Dashboard' || $designation_id_old == 'MisState' || $designation_id_old == 'DDO') {
      ...
      foreach ($roleArray as $roleObj) {
        if (in_array($roleObj['scheme_id'], array(2, 10, 11, 1, 3, 6, 5, 7, 8, 9, 13, 17, 19))) {
  ...
  public function getData(Request $request)
  {
    ...
    if ($validator->passes()) {
      ...
      if (!empty($gp_ward)) {
        ...
        $data = $this->getWardWise($district, $block, $muncid, NULL, $from_date, $to_date, $caste, $schema);
      } else if (!empty($block)) {
        if ($urban_code == 1) {
          $column = "Municipality";
          $heading_msg = 'Municipality Wise ' . $user_msg . ' of the Sub Division ' . $blk_munc_name;
          $data = $this->getMuncWise($district, $block, NULL, NULL, $from_date, $to_date, $caste, $schema);
          ...
          $column = "Block";
          $data = $this->getBlockWise($scheme_id, $district, NULL, NULL, NULL, $from_date, $to_date, $caste, $schema);
        }
      } else {
        ...
        if (!empty($district)) {
          if ($urban_code == 1) {
            ...
            $data = $this->getSubDivWise($scheme_id, $district, NULL, NULL, NULL, $from_date, $to_date, $caste, $schema);
          } else if ($urban_code == 2) {
            $heading_msg = 'Block Wise ' . $user_msg . ' of the District ' . $district_row->district_name;
            $column = "Block";
            $data = $this->getBlockWise($scheme_id, $district, NULL, NULL, NULL, $from_date, $to_date, $caste, $schema);
            ...
            $data1 = $this->getBlockWise($scheme_id, $district, NULL, NULL, NULL, $from_date, $to_date, $caste, $schema);
            $data2 = $this->getSubDivWise($scheme_id, $district, NULL, NULL, NULL, $from_date, $to_date, $caste, $schema);
            $data = array_merge($data1, $data2);
          }
        } else {
          $column = "District";
          $heading_msg = 'District Wise ' . $user_msg;
          $data = $this->getDistrictWise($scheme_id, NULL, NULL, NULL, NULL, $from_date, $to_date, $schema);
  ...
  public function getDistrictWise($scheme_id, $district_code = NULL, $ulb_code = NULL, $block_ulb_code = NULL, $gp_ward_code = NULL, $fromdate = NULL, $todate = NULL, $schema)
  {
  ...
  public function getSubDivWise($scheme_id, $district_code = NULL, $ulb_code = NULL, $block_ulb_code = NULL, $gp_ward_code = NULL, $fromdate = NULL, $todate = NULL, $caste = NULL, $schema)
  {
  ...
  public function getBlockWise($scheme_id, $district_code = NULL, $ulb_code = NULL, $block_ulb_code = NULL, $gp_ward_code = NULL, $fromdate = NULL, $todate = NULL, $caste = NULL, $schema)
  {
    $whereMain = "where  district_code=" . $district_code;
    $query = "select A.location_id,A.location_name,
        COALESCE(C.total_name_mismatch,0) as total_name_mismatch,
