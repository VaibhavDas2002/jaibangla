...
class DistrictDrillDownReport extends Controller
{
  ...
  public function payment(Request $request, $type)
  {
    ...
    return view('District-Drilldown.payment', [
      'schemes' => $schemes,
      'type' => $type,
      'districts' => $districts,
  ...
  public function getconsol_reportData(Request $request)
  {
    ...
    if (request()->ajax()) {
      ...
      $union_query = "SELECT tld.district_name,tld.district_code,
        ...
        coalesce(COUNT( distinct b.id) FILTER(WHERE date_part('year',b.created_at)='" . $pension_search_year . "' AND b.is_verified=1 and b.is_approved=0 and b.is_rejected=0 AND trim(to_char(b.created_at,'Month'))= '" . $month . "'),0) as verified,
        ...
        coalesce(COUNT( distinct b.id) FILTER(WHERE b.is_rejected=1 AND date_part('year',b.created_at)='" . $pension_search_year . "' AND trim(to_char(b.created_at,'Month'))= '" . $month . "'),0) as rejected,
        coalesce(COUNT( distinct b.id) FILTER(WHERE l.wrongdata_flag in (0,1,2) AND l.push_to_ifms_status=1),0) as pushed_ifms,
        coalesce(COUNT( distinct b.id) FILTER(WHERE l.wrongdata_flag in (1) AND (l.ref_no is not null or l.ref_no <> -1)),0) as ifms_returned,
        ...
        LEFT JOIN " . $table_name . " b ON d.district_code = b.dist_code AND b.scheme_id = " . $scheme_id . " LEFT JOIN 
        (SELECT e.pension_id,e.scheme_id,lm.ref_no, e.wrongdata_flag, e.amount, lm.push_to_ifms_status 
        FROM ifms.' . $archive_table . ' e LEFT JOIN lot_master lm 
        ON e.drn_part = lm.lot_no 
        AND lm.scheme_id = " . $scheme_id . " 
        AND lm.lot_year= '" . $year . "' 
        AND lm.lot_month= '" . $month . "' 
      ...
      if ($payment_option == 3) {
        $data = DB::connection('pgsql_main_mis')->select($union_query);
      } else {
        // print $query;die;
        $data = DB::connection('pgsql_main_mis')->select($query, $parameter);
      }
  ...
  public function getconsol_reportData_sbi(Request $request)
  {
    ...
    if (request()->ajax()) {
      ...
      coalesce(count( b.id) FILTER(WHERE date_part('year',b.created_at)='" . $pension_search_year . "' and b.is_verified=1 and b.is_approved=0 and b.is_rejected=0 and trim(to_char(b.created_at,'Month'))= '" . $month . "'),0) as verified,
      ...
      if ($payment_option == 3) {
        $data = DB::connection('pgsql')->select($union_query, $parameter);
      } else {
  ...
  public function excelDownloadFailedPaymentList(Request $request)
  {
    ...
    $data = $this->getFailed($scheme, $mode, $table_name,$district);

    $ben[] = array('Beneficiary Id', 'Name', 'Fathers Name', 'Ration Card No', 'Block/Municipality', 'Assembly', 'GP/Ward', 'Village/Town', 'Post office', 'PIN Code', 'Bank Acc No', 'IFSC Code', 'Status');

    foreach ($data as $arr) {
      $ben[] = array(
        'Beneficiary Id' => $arr->id,
        ...
        'PIN Code' => $arr->pincode,
        'Bank Acc No' => $arr->bank_code,
        'IFSC Code' => $arr->bank_ifsc, 
        'Status' => $arr->bank_edited==1?'Corrected':'Pending'
      );
    }

    Excel::create($schemeObj->scheme_name, function ($excel) use ($ben) {
      //  $excel->setTitle('Payment Status');
      $excel->sheet('Payment Success', function ($sheet) use ($ben) {
        $sheet->fromArray($ben, null, 'A1', false, false);
      });
    })->download('xlsx');
  }

  public function getFailed($scheme, $mode, $table_name, $district)
  {

    // $query = "select concat(ben_fname,' ',ben_mname,' ',ben_lname) as Name from pension.beneficiary limit 10 ";
    $lot_generated = "-1";
    if ($mode == 'RBI') {
      $lot_generated = "-2";
      // $bank_edited = "1";
    ...
    $data = DB::connection('pgsql')->table($table_name)->where('created_by_dist_code',$district)
    ->where('scheme_id',$scheme)->where('next_level_role_id', 0)->where('lot_generated', $lot_generated)
    ->get();
    // dd($data);
    return $data;
  }
