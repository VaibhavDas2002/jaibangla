...
class DuplicateAadharUpdateController extends Controller
{
  ...
  public function getDuplicateAadharListView(Request $request)
  {
    ...
    if ($request->ajax()) {
      ...
      if ($mapping_level == 'Block') {
        $query = "select * from " . $table_name . " where (is_rejected=0) and ((dup_aadhar = 1 and dup_aadhar_edit_role_id IS NULL ) or (dup_mobile = 1 and dup_mobile_edit_role_id IS NULL) OR (no_mobile = 1 AND no_mobile_edit_role_id IS NULL)) and scheme_id = " . $scheme_id . " and created_by_dist_code = " . $district_code . " and created_by_local_body_code = " . $urban_body_code . " ";
      ...
      return datatables()->of($data)
        ->addColumn('view', function ($data) {
          ...
          if ($data->payment_suspended == 1) {
            ...
            if ($data->no_mobile == 1 and $data->no_mobile_edit_role_id == '') {
              $action .= '&nbsp; &nbsp; <button onclick=editNoMobileFunction(' . $data->id . ',' . $data->scheme_id . ') class="btn btn-xs btn-success" title="Update No Mobile Number"><i class="glyphicon glyphicon-edit"></i> Edit No Mobile</button>';
            }
          }
          return $action;
        })
      ...
      if ($validator->passes()) {
        ...
        try {
          ...
          if ($is_upload == 1) {
            ...
            // $UpdateBenDetailsInsert=1;
            $UpdateBenDetailsInsert = DB::table('public.update_ben_details')->insert($updateBenDetailsData);
            // $update = DB::table($beneficiary_table)->where('id', $ben_id)->update();
            if ($UpdateBenDetailsInsert) {
              $finalBenTable = DB::table($beneficiary_table)->where('scheme_id', $scheme_id)->where('id', $ben_id)->update($beneficiaryUpdate);
              // $finalBenTable=1;
              if ($finalBenTable) {
      ...
      if ($validator->passes()) {
        ...
        try {
          ...
          if ($is_insert) {
            ...
            $paymentUpdate['mobile_no'] = $new_mobile_no;
  ...
  */
  ...
  function updateNoBenMobileDetails(Request $request)
  {
    ...
    try {
      $rules = array(
        'new_no_mobile_no' => 'required|numeric|digits:10',
        'no_mob_remarks' => 'max:100'
      );
      $attributes = [
        'new_no_mobile_no' => 'Mobile Number',
        'no_mob_remarks' => 'Remarks'
      ];
      $messages = [
        'required' => 'The :attribute field is required.',
        'numeric' => 'The :attribute field is allow only numbers.',
      ...
      if ($validator->passes()) {
        $scheme_id = $request->scheme_id;
        $ben_id = $request->id;
        $old_no_mobile_no = $request->old_no_mobile_no;
        ...
        $input['mobile_no'] = $new_no_mobile_no;
        $ip_address = request()->ip();
        $updateBenDetailsData = [
          'original_application_id' => $benDetails->id,
          'dist_code' => $benDetails->dist_code,
