        ...
        if ($status = $this->schemeSessionCheck($request)) {
            ...
            if (request()->ajax()) {
                ...
                return datatables()
                    ...
                    ->rawColumns(['ben_id', 'ben_name', 'aadhar_no','ben_age',  'mobile_no','action','aadhar_no_rw','check'])
                    ->make(true);
            } else {
                //Goto view
 ...
 public function bulkAccept(Request $request)
    {
      ...
      $inputs = request()->input('check_beificieries');
      //dd($inputs);
      $c_time = date('Y-m-d H:i:s', time());
    ...
    //   }
     ...
     //dd($in_pension_id);
      try {
        DB::beginTransaction();
        $is_inserted_status_arr = DB::select("select ".$schema.".no_aadhaar_validation_request_accept(in_ip => '".$ip."' , in_application_id => $in_pension_id,in_scheme_id => $scheme_id,in_district_code => $district_code,id_created_by_local_body_code=> $created_by_local_body_code,in_user_id => $user_id, in_op_type => '".$accept_reject."', in_custom_comment => '".$comments."')");
        $is_inserted_status=$is_inserted_status_arr[0]->no_aadhaar_validation_request_accept;

        if($is_inserted_status==1){
            DB::commit();
            return redirect($back_url)->with('success', 'Applications NO Aadhaar Allow request has been Accepted Succesfully!');
