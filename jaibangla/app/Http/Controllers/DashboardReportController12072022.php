...
class DashboardReportController extends Controller
{
  ...
  public function markDuplicateAccountNumber(Request $request) {
    ...
    try {
      ...
      select m.scheme_name,m.id as scheme_id,coalesce(sum(d.total),0) as total from Duplicates d JOIN m_scheme m ON d.scheme_id=m.id WHERE m.id IN(2,10,11,3,8,9,17) group by m.scheme_name,m.id";
  ...
  private function getAllCounts($scheme_id, $table_name, $district_code, $body_code, $scheme_name) { 
    ...
    COALESCE(count( b.id) FILTER(WHERE b.next_level_role_id > 0 ),0) as approval_pending,
    ...
    COALESCE(count( b.id) FILTER(WHERE b.next_level_role_id = -97 ),0) as bank_ac_dup,
    ...
    COALESCE(count( b.id) FILTER(WHERE (next_level_role_id=0 AND lot_generated=0) OR (b.next_level_role_id = 0 AND lot_generated IN(-1,-2,-3) AND bank_edited=1) ),0) as lot_not_generated
    ...
    $fHtml .= 'Applied : ' . $data[0]->applied . '</br>';
    $fHtml .= 'Verification Pending : ' . $data[0]->verification_pending . '</br>';
    $fHtml .= 'Approval Pending : ' . $data[0]->approval_pending . '</br>';
    $fHtml .= 'Total Rejected : ' . $data[0]->rejected . '</br>';
    $fHtml .= '</div><div class="col-md-4">';
    $fHtml .= 'Approved : ' . $data[0]->approved . '</br>';
    $fHtml .= 'Duplicate Bank A/c : ' . $data[0]->bank_ac_dup . '</br>';
    $fHtml .= 'Bank Faliure : ' . $data[0]->total_failed . '</br>';
    $fHtml .= 'Bank Rectification Pending : ' . $data[0]->total_failed_not_edited . '</br>';
    $fHtml .= '</div><div class="col-md-4">';
    $schemeArrAfterStandardLot = array(2,10,11,8,9,17,13);
    if (in_array($scheme_id, $schemeArrAfterStandardLot)) {
      $fHtml .= 'Payment Ongoing : ' . $data[0]->payment_ongoing . '</br>';
      $fHtml .= 'Pending Response from bank : ' . $data[0]->pending_response . '</br>';
    }
    else {
      $fHtml .= 'Pending Ongoing : ' . $data[0]->pending_response . '</br>';
    }
  ...
  public function getInstantPaymentReport(Request $request) {
    ...
    try {
      ...
      $lotQuery = "SELECT SUM(rbi_success_count) AS success,SUM(rbi_failed_count) AS failed FROM public.lot_master WHERE lot_status>0 and scheme_id=" . $scheme_id . " and lot_month='" . $month . "' and lot_year='" . $year . "'";
      ...
      FROM " . $table_name . " b WHERE scheme_id=" . $scheme_id . "";
      $lotData = DB::connection('pgsql_mis')->select($lotQuery);
      $preData = DB::connection('pgsql_mis')->select($preQuery);
      $fHtml = '';
      $fHtml .= '<div style="font-size: 15px; padding-top: 10px;">';
      $fHtml .= 'Total Success : ' . $lotData[0]->success . '</br>';
      $fHtml .= 'Total Failed : ' . $lotData[0]->failed . '</br>';
      $fHtml .= 'Total Amount Disbursed(In Lakh.) : ' . ($lotData[0]->success*1000)/100000 . '</br>';
