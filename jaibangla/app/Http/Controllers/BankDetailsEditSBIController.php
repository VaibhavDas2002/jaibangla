...
class BankDetailsEditSBIController extends Controller
{
  ...
  public function generalfunc($request)
  {
    ...
    if ($is_first) {   // First Level Verifier   	
      if ($mappingLevel == "State") {
        ...
        $rows = DB::connection('pgsql_mis')->table($table_name)->where('lot_generated', -3)
        ...
        if ($is_urban == 1) {
          ...
          if (request()->ajax()) { // dd($urban_body_code);
            if (!empty($request->filter_1)  && empty($request->filter_2)) { //dd($urban_body_codes);     
              ...
              $data = DB::connection('pgsql_mis')->table($table_name)->where(function ($query) use ($role_id) {     //29-07-2020
          ...
          $appPrefix = "App";
          $modelName = $appPrefix . "\\" . $ben_table;
          $rows = DB::connection('pgsql_mis')->table($table_name)->where('next_level_role_id', 0)
            ->where('lot_generated', -3)
            ->where('scheme_id', $scheme_id)
      ...
      if ($mappingLevel == "State") {
        ...
        if (request()->ajax()) {
          if (!empty($request->filter_1) && !empty($request->filter_2)) {
            ...
            $appPrefix = "App";
            $modelName = $appPrefix . "\\" . $ben_table;
            $data = DB::connection('pgsql_mis')->table('pension.beneficiaries')->where('created_by_dist_code', $district_code)
        ...
        if ($is_urban == 1) {
          ...
          $rows = $data = DB::connection('pgsql_mis')->table('pension.beneficiaries')->where('next_level_role_id', $role_id)->where('created_by_local_body_code', $taluka_code)->orderBy('id', 'desc')->paginate(10);
          //return view('pension_list',['nhm_employee_details' => $rows]);
  ...
  public function editApplicantDetails(Request $request)
  {
    ...
    $single_employee_details = DB::connection('pgsql_mis')->table($table_name)->where('id', '=', $id)
      ->where('scheme_id', $scheme_id)
      ->where(function ($query) use ($role_id) {
        $query->where('next_level_role_id', $role_id)
          ->orWhere('next_level_role_id', 0);
    ...
    $invalid_status='';
$query_res= DB::connection('pgsql_mis')->select($query);
if(!empty($query_res[0]->description)){
  ...
  public function update(Request $request)
  {
    ...
    if ($_POST['submit'] == 'Update') {
      $benDetails = DB::connection('pgsql_mis')->table('pension.beneficiaries')->where('id', $id)->first();

      // Check Duplicate Account No
      $schemeId = $benDetails->scheme_id;
      if ($schemeId == 2 || $schemeId == 10 || $schemeId == 11) {
          $benDuplicateAcCount = DB::connection('pgsql_mis')->table('pension.beneficiaries')->whereRaw("trim(bank_code)=trim("."'".$bank_code."'".")")->whereRaw("trim(bank_ifsc)=trim("."'".$bank_ifsc."'".")")
          ->where('id', '!=', $id)
          ->where('is_rejected',0)
