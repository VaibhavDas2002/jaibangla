...
class JBDocUploadController extends Controller
{
  ...
  public function shemeSelection(Request $request)
  {
    ...
    if ($designation_id == 'Delegated Verifier' ||  $designation_id == 'Delegated Approver' || $designation_id == 'Operator' || $designation_id == 'Verifier' || $designation_id == 'Approver') {
  ...
  public function ListView(Request $request)
  {
    try {
      ...
      if ($duty_obj->mapping_level == "Subdiv" || $duty_obj->mapping_level == "Block") {
        ...
        if (empty($serachvalue)) {
          ...
          $data = $query->orderBy('dob', 'ASC')->offset($offset)->limit($limit)->get([
            ...
            'is_rejected','is_verified','is_approved'
        ...
        return datatables()->of($data)->setTotalRecords($totalRecords)
          ...
          ->addColumn('status', function ($data) {
            $status='';
            if($data->is_verified==1 && $data->is_approved==0){
  ...
  public function View(Request $request)
  {
    try {
      ...
      if ($duty_obj->mapping_level == "Subdiv" || $duty_obj->mapping_level == "Block" || $duty_obj->mapping_level == "District"){
      $query = DB::table('pension.beneficiaries')
        ->where('created_by_dist_code', $district_code)
        ->where('id', $request->id)->where('is_rejected',0)->where('scheme_id',$scheme_id);
      }
      else{
        $query = DB::table('pension.beneficiaries')
        ->where('id', $request->id)->where('is_rejected',0)->where('scheme_id',$scheme_id);
