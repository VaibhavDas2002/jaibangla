	...
	{ //dd($request->all());
		...
		if (array_intersect([2, 8, 9, 17, 10, 11, 13, 3, 7, 6], $schemearray)) {
			...
			try {
				...
				// echo $lot_size.'-'.$check; die;
				if (($lot_size == 0) || ($lot_size == '')) {
					return $response = array(
						'status' => 4, 'title' => 'Error', 'type' => 'red', 'icon' => 'fa fa-warning', 'content' => 'Please enter correct beneficiary count.'
					);
					exit();
				}
				// 	echo 1;die;
				...
				$logmessage .= "------------------------------" . "\n";
				$logmessage .= "Lot Type-> " . $select_lot_type . ", Lot Year-> " . $select_year . ", Lot Month-> " . $select_month . ", Scheme Id-> " . $scheme_id . "\n";
				$logmessage .= 'Exception - ' . $e->getMessage();
				$fileLocation = 'lotCreationLog/lotCreationLog.txt';
			...
			$report = $report->where('lot_month', $getmonthValue['month'])->where('scheme_id', $scheme_id)
				...
				if (!empty($new_lot_no['exception_message'])) {
					echo "<pre>";
					print_r($new_lot_no['exception_message']);
					die;
					return redirect("generic-lot")
...
*/
	...
	{ //dd($request->all());
		...
		//Get Schema Name
		$schema_name_short_code = Scheme::where('id', $select_scheme)->value('short_code');
		$schema_name = strtolower($schema_name_short_code);
		...
		try {
			...
			if ($select_lot_type != 2) {
				...
				if ($select_lot_type == 1) {
					$bencount = DB::table($schema_name_short_code . '.beneficiary')->where('payment_count', 0)->where('next_level_role_id', 0)->where('lot_generated', 0)
						->where('last_paid_yymm', '0');
					if (!empty($select_month)) {
						if ($select_scheme == 10 || $select_scheme == 11) {
							$bencount = $bencount->whereMonth('payment_start_date', $monthVal);
						}
						else {
					...
					if (!empty($select_year)) {
						if ($select_scheme == 10 || $select_scheme == 11) {
							$bencount = $bencount->whereYear('payment_start_date', $yearVal);
						}
						else {
					...
					$bencount = DB::table($schema_name_short_code . '.beneficiary')->where('next_level_role_id', 0)
						...
						if ($select_scheme == 10 || $select_scheme == 11) {
							$bencount = $bencount->where(function ($query) use ($getmonthValue, $monthVal, $yearVal) {
								...
								$query->orwhere(function ($query) use ($monthVal, $yearVal) {
									$query->where('payment_count', '=', 0)
										->whereMonth('payment_start_date', $monthVal)
										->whereYear('payment_start_date', $yearVal);
								});
							});
						} else {
							$bencount = $bencount->where(function ($query) use ($getmonthValue, $monthVal, $yearVal) {
					...
					$bencount = DB::table($schema_name_short_code . '.beneficiary')->where('payment_count', '>', 0)->where('next_level_role_id', 0)->where('lot_generated', 2)
						->where('last_paid_yymm', $getmonthValue['chk_paid_yymm']);
				...
				// if (in_array($select_scheme, $schemeArr)) {
				// 	$bencount = $bencount->where('acc_validated_payment', 10);
				// }
				$bencount = $bencount->where('dup_bank', 0)->where('dup_bank_pending', 0)->whereNull('payment_suspended')->count();
			}
	...
	public function getSchemeWiseLot(Request $request)
	{
		...
		try {
			$select_scheme = $request->select_scheme;
			if (!empty($select_scheme)) {
				$query = "select * from public.m_lot_type where id in(select unnest(lot_type_id) as lot_type_id from public.m_scheme where id=" . $select_scheme . ")";
				// if ($select_scheme == 10 || $select_scheme == 11) {
				// 	$query .= " and id <> 1";
				// }
	...
	public function getAmountMonthWise(Request $request)
	{
		$statusCode = 200;
		$response = [];
		if (!$request->ajax()) {
		...
		try {
			...
			$month = array_search($select_month, Config::get('constants.monthval'));
			if ($month < 4) {
				$year = substr($select_year, 7, 2);
			} else {
				$year = substr($select_year, 2, 2);
			}
			$yymm_value = $year . str_pad($month, 2, "0", STR_PAD_LEFT);

			if (!empty($select_scheme)) {
				$amount = 0;
				if ($select_scheme == 17) {
					$amountMasterObj = DB::table('public.m_lot_amount')->where('scheme_id', $select_scheme)->where('from_yymm', '<=', $yymm_value)->where('to_yymm', '>=', $yymm_value)->first();
					$amount = $amountMasterObj->amount;
				} else {
					$amount = 1000;
				}
			} else {
