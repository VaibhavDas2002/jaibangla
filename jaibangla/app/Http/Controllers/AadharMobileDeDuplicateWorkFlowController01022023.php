  ...
  public function verifyDeDuplicateAadharMobileGetData(Request $request)
  {
    if ($request->ajax()) {
      ...
      if ($mapping_level == 'Block') {
        $query = '';
        $query = "select * from " . $table_name . " where (next_level_role_id > 0 or next_level_role_id is null or next_level_role_id = 0 or next_level_role_id =-97) and ((dup_aadhar = 1 and dup_aadhar_edit_role_id = 1) or (dup_mobile = 1 and dup_mobile_edit_role_id = 1)) and scheme_id = " . $scheme_id . " and created_by_dist_code = " . $district_code . " and created_by_local_body_code = " . $urban_body_code . " ";
  ...
  */
  public function approveDeDuplicateGetData(Request $request)
  {
    if ($request->ajax()) {
      ...
      $query = '';
      $query = "select * from " . $table_name . " where (next_level_role_id > 0 or next_level_role_id is null or next_level_role_id = 0 or next_level_role_id =-97) and scheme_id = " . $scheme_id . " and created_by_dist_code = " . $district_code . " ";
