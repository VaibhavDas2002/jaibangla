    ...
    function editicadPost(Request $request)
    {
        ...
        if ($validator->passes()) {
            ...
            if(!empty($request->aadhar_no)){
                $count_aadhar =  DB::table($schema . '.beneficiary')->where('aadhar_no', trim($request->aadhar_no))->where('id', '!=', $id)->whereRaw("(" . $check_condition_str . ")")->count('id');
                if ($count_aadhar > 0) {
                    $errors = array();
            ...
            if(!empty($request->mobile_no)){
                $count_mobile = DB::table($schema . '.beneficiary')->where('mobile_no', $request->mobile_no)->where('id', '!=', $id)->whereRaw("(" . $check_condition_str . ")")->count('id');
                if ($count_mobile > 0) {
                    $errors = array();
            ...
            try {
                            ...
                            if($is_inserted_status==1){
                               ...
                               bank_edited, bank_code, payment_count, last_paid_yymm, 
                               av_status,   legacy_import, old_beneficiary_id, pensioner_type
                                 from ".$schema.".beneficiary where id=" . $id . ")");
                                 //dd($arch_status);
                               if($arch_status)
                               {
                               ...
                               // dd($main_update);
                                if($main_update){
                                   ...
                                   // dump($doc_inserted_arch); dump($doc_inserted_del);dd($doc_inserted);
