...
class ReportLotMasterExcelGenerateController extends Controller
{
  ...
  public function lotWiseGenerateExcelFunction(Request $request)
  {
    ...
    // echo '<pre>'; print_r($payModeObj);die;
    ...
    if ($payModeObj->payment_mode == 'IFMS') {
      $ifms_transaction_lot = DB::table('ifms.transaction_lot')->where('lot_no', $lot_no)->where('scheme_id', $scheme_id)->value('lot_year');
      $fin_arr = explode("-", $ifms_transaction_lot);
      $year1 = substr($fin_arr[0], 2);
      $year2 = substr($fin_arr[1], 2);
      $lot_year = $year1 . $year2;

      $ifms_tld_report = DB::table('ifms.transaction_lot_details')->where('drn_part', $lot_no)
        ->where('scheme_id', $scheme_id)->count();
      if ($type == 'E1') {
        if ($ifms_tld_report > 0) {
          $query_ifms = "select m.district_name,pb.block_ulb_name,  be.pension_id as pension_id, be.name as name, be.acc_no as acc_no, be.ifsc as ifsc, be.mobile_no as mobile_no, be.ifms_status as err_reason, be.utr_no as utr_no
                    from ifms.transaction_lot_details be  join m_district m
                    on m.district_code=be.dist_code join " . $table . " pb 
                    on pb.id=be.pension_id where wrongdata_flag=1 and ifms_ref_no=0 and drn_part='" . $lot_no . "' and be.scheme_id=" . $scheme_id . " order by m.district_name";
          ...
          $query_ifms = "select m.district_name,pb.block_ulb_name,  be.pension_id as pension_id, be.name as name, be.acc_no as acc_no, be.ifsc as ifsc, be.mobile_no as mobile_no, be.ifms_status as err_reason, be.utr_no as utr_no
                  from ifms.transaction_lot_details_report_" . $lot_year . " be  join m_district m
                  on m.district_code=be.dist_code join " . $table . " pb 
                  on pb.id=be.pension_id where wrongdata_flag=1 and ifms_ref_no=0 and drn_part='" . $lot_no . "' and be.scheme_id=" . $scheme_id . " order by m.district_name";
          // $query_ifms="select be.pension_id as pension_id, be.name as name, be.acc_no as acc_no, be.ifsc as ifsc, be.mobile_no as mobile_no, be.ifms_status as err_reason, be.utr_no as utr_no  
        ...
        if ($ifms_tld_report > 0) {
          ...
          $query_ifms = "select be.pension_id as pension_id, be.name as name, be.acc_no as acc_no, be.ifsc as ifsc, be.mobile_no as mobile_no, (case when trim(be.ifms_status)='Payment Mandate Generated' then 'SUCCESS' else be.ifms_status end) as err_reason, be.utr_no as utr_no 
				from ifms.transaction_lot_details_report_" . $lot_year . " be where wrongdata_flag=0 and ifms_ref_no>0 and drn_part='" . $lot_no . "' and scheme_id=" . $scheme_id . ";";
        }
        ...
        if ($ifms_tld_report > 0) {
          ...
          $query_ifms = "select be.pension_id as pension_id, be.name as name, be.acc_no as acc_no, be.ifsc as ifsc, be.mobile_no as mobile_no, (case when trim(be.ifms_status)='Payment Mandate Generated' then 'SUCCESS' else be.ifms_status end) as err_reason, be.utr_no as utr_no 
                    from ifms.transaction_lot_details_report_" . $lot_year . " be where drn_part='" . $lot_no . "' and scheme_id=" . $scheme_id . ";";
        }
        $result = DB::select(DB::raw($query_ifms));
    ...
    // ===================
      ...
      $sbi_transaction_lot = DB::table('sbi.transaction_lot')->where('lot_no', $lot_no)->where('scheme_id', $scheme_id)->value('lot_year');
      $fin_arr = explode("-", $sbi_transaction_lot);
      $year1 = substr($fin_arr[0], 2);
      $year2 = substr($fin_arr[1], 2);
      $lot_year = $year1 . $year2;

      $sbi_tld_report = DB::table('sbi.transaction_lot_details')->where('lot_no', $lot_no)->where('scheme_id', $scheme_id)->count();
      ...
      on m.district_code=pb.created_by_dist_code 
          ...
          $query_sbi = "select m.district_name,pb.block_ulb_name, tld.pension_id as pension_id, tld.name as name, tld.account_credit as acc_no, tld.ifsc_code_credit as ifsc,
                (select error.description as status from sbi.credit_transaction_code as error where error.code=tld.status_code) 
                from sbi.transaction_lot_details_report_" . $lot_year . " tld join " . $table . " pb 
      on pb.id=tld.pension_id join m_district m
      on m.district_code=pb.created_by_dist_code 
                where status_code like '%E%' and debit_reference in
                (select debit_reference from sbi.transaction_lot where lot_no='" . $lot_no . "');";
        ...
        if ($sbi_tld_report > 0) {
          ...
          $query_sbi = "select tld.pension_id as pension_id, tld.name as name, tld.account_credit as acc_no, tld.ifsc_code_credit as ifsc,
					(select error.description as status from sbi.credit_transaction_code as error where error.code=tld.status_code) 
					from sbi.transaction_lot_details_report_" . $lot_year . " tld 
					where status_code like '%S%' and debit_reference in
					(select debit_reference from sbi.transaction_lot where lot_no='" . $lot_no . "');";
        }
        ...
        if ($sbi_tld_report > 0) {
          ...
          $query_sbi = "select tld.pension_id as pension_id, tld.name as name, tld.account_credit as acc_no, tld.ifsc_code_credit as ifsc,
                    (select error.description as status from sbi.credit_transaction_code as error where error.code=tld.status_code) 
                    from sbi.transaction_lot_details_report_" . $lot_year . " tld 
                    where is_active=1 and debit_reference in
                    (select debit_reference from sbi.transaction_lot where lot_no='" . $lot_no . "');";
        }
        $result = DB::select(DB::raw($query_sbi));
