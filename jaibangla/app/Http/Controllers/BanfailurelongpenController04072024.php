...
class BanfailurelongpenController extends Controller
{
        ...
        if (empty($serachvalue)) {
          ...
          $data = $query->orderBy('id', 'ASC')->offset($offset)->limit($limit)->get([
            'id', 'created_by_dist_code', 'dob', 'assembly_name',
                ...
                'scheme_id', 'lot_generated', 'payment_count', 'next_level_role_id', 'aadhar_edit_role_id'
          ]);
    ...
    try{
     ...
     // dd($id);
      ...
      $query = DB::table($schema . '.beneficiaries')
      ->where('bank_edited',0)->whereIn('lot_generated',[-1,-2,-3])->where('created_by_dist_code', $district_code)->where('id',$id)->where('next_level_role_id',0);
      $row = $query->first();
      //dd( $row);
      if (empty($row)) {
        return redirect("/")->with('danger', 'Not Allowed');
    ...
    public function bulkApprove(Request $request)
    {
      ...
      $inputMain = [
        'next_level_role_id' => -30,'is_approved' => 2,'is_verified' => 2,'is_rejected' => 1,
        'rejected_date' => $c_time,'rejected_by' => $user_id,'is_clean' => 10
      ];
      $upadated_main = DB::table($schema . '.beneficiaries')->whereIn('id',$applicationid_arr)->where('bank_edited',0)
          ->where('next_level_role_id',0)->where('created_by_dist_code',$district_code)->whereIn('lot_generated',[-1,-2,-3])->update($inputMain); 
