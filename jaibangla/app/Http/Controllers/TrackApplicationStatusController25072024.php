  ...
  */
  public function getTrackApplicantDetails(Request $request)
  {
    if ($request->ajax()) {
      ...
      $query = '';
      $query = "select b.*, md.district_name, m.district_name, md.district_name as loc_dist, bl_div.block_subdiv_name, ms.scheme_name,  
        CASE 
      ...
      return datatables()->of($data)
        ...
        ->rawColumns(['name', 'father_name', 'address', 'bank_info', 'current_status', 'payment_info', 'view_payment_status', 'scheme_name', 'action_needs_to_taken'])
        ->make(true);
    }
  }
    ...
    try {
      ...
      if (isset($benPaymentDetails)) {
        ...
        if ($benPaymentDetails->pay_validated == 3) {
          $benAccSt = 'Payment transaction failed (SBI Failed)';
        } else if ($benPaymentDetails->pay_validated == 4) {
          $benAccSt = 'Payment transaction failed (RBI Failed)';
        } else if ($benPaymentDetails->pay_validated == 5) {
          $benAccSt = 'Payment transaction failed (IFMS Failed)';
        } else {
          $benAccSt = 'Ready For Payment';
        }

        $benValSt = ($benValSt != '') ? 'Account Validation Status : ' . $benValSt . '<br>' : '';
        $paymentDetailsHtml .= '<span class="text-info" style="font-weight: bold;">' . $benValSt . 'Payment Status : ' . $benAccSt . '</span>';
      } else {
