...
class RBIRevertBackController extends Controller
{
  ...
  public function shemeSessionCheck(Request $request)
  {
    ...
    if ($request->get('pr1')) {
      ...
      return redirect("scheme-selection-revert-rbi/main");
    }
    ...
    if ($is_first) {   // First Level Verifier   	
      ...
      if ($mappingLevel == "State") {
        ...
        $rows = $modelName::where('lot_generated', -1)
          ...
          ->where('payment_count', 0) // First Time RBI Check
          ->paginate(10);
        return view('scheme-selection-revert-rbi/main');
        ...
        if ($is_urban == 1) {
          ...
          if (request()->ajax()) {
            ...
            return datatables()->of($data)
              ->addColumn('view', function ($data) {
                return '<a href="' . route('revert.rbieditApplicantDetails', $data->id) . '" class="btn btn-ls btn-primary"><i class="glyphicon glyphicon-edit"></i> View</a>';
              })
          ...
          return view('linelisting_reverted_subdiv_rbi')->with('duty_level', $duty_level)->with('urban_bodys', $urban_bodys)->with('dist_code', $district_code)->with('scheme_name', $schemeObj->scheme_name);;
        } else {
        ...
        if (request()->ajax()) {
          ...
          return datatables()->of($data)
            ->addColumn('view', function ($data) {
              return '<a href="' . route('revert.rbieditApplicantDetails', $data->id) . '" class="btn btn-ls btn-primary"><i class="glyphicon glyphicon-edit"></i></a>';
            })
            ->addColumn('id', function ($data) {
              return $data->getBenidAttribute();
            })
        ...
        /*echo "<pre>";
                     ...
                     echo "</pre>";
                     die();*/
        return view('linelisting_reverted_rbi')->with('duty_level', $duty_level)
          ->with('gps', $gps)
...
//echo "<pre>"; print_r($query_res);die;
//echo $query_res[0]->ifms_status;die;
    return view('pension-revert-edit-rbi', ['row' => $single_employee_details,'invalid_status'=>$invalid_status]);
  ...
  public function update(Request $request)
  {
    ...
    if ($_POST['submit'] == 'Update') {
      ...
      if ($schemeId == 2 || $schemeId == 10 || $schemeId == 11) {
          $benDuplicateAcCount = BeneficiaryPensions::whereRaw("trim(bank_code)=trim("."'".$bank_code."'".")")->whereRaw("trim(bank_ifsc)=trim("."'".$bank_ifsc."'".")")
          ->where('id', '!=', $id)
          ->where('is_rejected',1)
          ->where('scheme_id', $schemeId)
          ->count('id');
          // dd($benDuplicateAcCount);
          // echo $benDuplicateAcCount;
          if($benDuplicateAcCount > 0)
          {
              $msg = 'This Bank A/c - '.$bank_code.' & IFSC - '.$bank_ifsc.' already exist in this scheme';
              return redirect('scheme-selection-revert-rbi')->with('error', $msg)->with('pr1', $pr1);
          }
      }
      // echo $benDuplicateAcCount; 
      // echo 1; die();
      ...
      if ($is_status_updated) {
        return redirect('scheme-selection-revert-rbi')->with('success', 'Bank Details Updated Succesfully!')->with('pr1', $pr1);
      } else {
        DB::rollback();
        return redirect('scheme-selection-revert-rbi')->with('error', 'Oops! Bank Details Updation Failed.')->with('pr1', $pr1);
      }
    }
  }
