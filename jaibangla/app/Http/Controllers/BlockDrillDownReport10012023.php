  ...
  echo json_encode($json_data);
    ...
    public function getlistsubmitted($block_code, $scheme_id)
    {
        ...
        $results = BeneficiaryPensions::where('created_by_local_body_code', $block_code)->where('scheme_id', $scheme_id)
            ...
            ->orderby('pension.beneficiary.id')->get(); //dd($results);
        //dd($results);
    ...
    public function getconsol_reportData(Request $request)
    {
        ...
        if (request()->ajax()) {
            ...
            coalesce(count(distinct b.id) FILTER(WHERE b.next_level_role_id >= 0),0) as verified,
    ...
    public function getwcdconsol_reportData(Request $request)
    {
        ...
        if (request()->ajax()) {
            ...
            if (!empty($district_code)) {
              ...
              from manabik.beneficiaries b, m_scheme s
            ...
            where s.id=b.scheme_id " . $condition .
                        " and scheme_id = 10 group by s.scheme_name,b.block_ulb_name,b.rural_urban_id
            order by b.rural_urban_id, b.block_ulb_name,s.scheme_name";
...
coalesce(count( distinct main.id) FILTER(WHERE next_level_role_id>0 and bank_code IS NOT NULL and next_level_role_id!=9999)) as verified,
coalesce(count( distinct main.id) FILTER(WHERE next_level_role_id<0)) as rejected,
main.gp_ward_code
                    from " . $table_schema . ".beneficiaries as main 
                    
                    where  main.block_ulb_code=" . $block . "  " . $scheme_condition . "
...
main.gp_ward_code
                ...
                (
                    ...
                    main.block_ulb_code
                    from " . $table_schema . ".beneficiaries as main 
                    where  main.created_by_dist_code=" . $district . "  " . $scheme_condition . "
...
main.block_ulb_code
                ...
                (
                    ...
                    from " . $table_schema . ".beneficiaries as main 
                    where  created_by_dist_code IS NOT NULL " . $scheme_condition . "
    ...
    function getAllSbiPaymentReport($table_name,$scheme_id,$schemes_in,$year,$month,$user_id){ 
        ...
        coalesce(count( b.id) FILTER(WHERE b.next_level_role_id >= 0),0) as to_be_approved,
        ...
        $query = $query . ") b 
        left join m_urban_body mb on mb.urban_body_code=b.block_ulb_code 
        LEFT JOIN (select tld.pension_id,tld.scheme_id,tl.lot_status, tld.credit_amount,tld.status_code  from sbi.transaction_lot_details_report tld right join sbi.transaction_lot tl on tld.lot_no = tl.lot_no 
         ";
        if (!is_null($year)) {
            $query = $query . ' and tl.lot_year= :lot_year';
        }
