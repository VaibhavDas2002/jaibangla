...
class DuplicateAadharMobileController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return view(
            'duplicate-aadhar-mobile-no/duplicate-aadhar-mobile',
            [
                'schemes' => $schemes,
                'districts' => $districts,
    ...
    public function linelistingReport(Request $request)
    {
        ...
        if ($validator->passes()) {
            $user_msg = $schemeObj->scheme_name." De-duplicate Aadhar and Mobile Report";
            $title = $user_msg;
            $return_status = 1;
            $return_msg = '';
            $heading_msg = '';
    ...
    public function getDistrictWise($scheme_id, $district_code = NULL, $ulb_code = NULL, $block_ulb_code = NULL, $gp_ward_code = NULL)
    {
        ...
        COALESCE(dup.aadhar_not_entered,0) as aadhar_not_entered,
        COALESCE(dup.de_dup_aadhar,0) as de_dup_aadhar,
        COALESCE(dup.mobile_not_entered,0) as mobile_not_entered,
        COALESCE(dup.de_dup_mobile,0) as de_dup_mobile 
        from
        (
        ...
        (
            select 
            count(1) filter(where no_aadhar=1 ) as aadhar_not_entered,
            count(1) filter(where dup_aadhar=1 ) as de_dup_aadhar,
            count(1) filter(where no_mobile=1 ) as mobile_not_entered,
            count(1) filter(where dup_mobile=1 ) as de_dup_mobile,
    ...
    public function getBlockWise($scheme_id, $district_code = NULL, $ulb_code = NULL, $block_ulb_code = NULL, $gp_ward_code = NULL)
    {
        ...
        (
            ...
            from ".$table_name."  ".$whereCon." and (is_rejected=0) 
